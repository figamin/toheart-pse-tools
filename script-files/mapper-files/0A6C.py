# Generated array of Japanese text with English comments
# Source file: 0A6C.txt

text_array = [
    ("（Ｂ　あかりといっしょに行く。）",
    ""),
    # (B: Go to Akari and Isshii.)

    ("「雛山さん、急にどうしちゃったんだろ？」",
    "What's gotten into her all of a sudden?"),
    # "Bath Hinayama-san, what's the sudden phlegm? b?"

    ("「気をつかったんじゃねーか？」",
    "She was probably trying to give us space or something."),
    # "You're taking care of me, are you phlegm?"

    ("「え、私たちに？」",
    "Us? Why?"),
    # "What, п?"

    ("「昔からよくあることだろ？",
    "It's happened plenty of times before."),
    ("いまさら説明するのもおっくうだぜ」",
    "It'd be too much of a drag to explain it now."),
    # "It's been a common occurrence for a long time? I'm not too excited to explain it now."

    ("「でも、勘違いされたままでいいの？」",
    "You sure you don't wanna clear the air with her?"),
    # "And is it okay if you leave Hoo in a misunderstanding too?"

    ("「べつに。",
    "I'm sure."),
    # "Additionally.

    ("う慣れたよ」",
    "I'm used to this happening."),
    # I'm used to it now"

    ("あのときのまま気が変わっていなければ、理緒ちゃんはオレのことが好きなはずだ。",
    "If Rio hasn't changed her mind since her confession, she would still have feelings for me."),
    # If she hadn't changed as she was back then, then Erie Ochi Sphin-yan would love me.

    ("そんな理緒ちゃんにすれば、オレとあかりの関係は、気になって仕方ないとこだろう。",
    "Someone like that would naturally be very curious about my relationship with Akari."),
    # If I were to use this as a stimulating stimulus, I was curious about the relationship between me and Akari, so I was sure I had to deal with her.

    ("すぐに追いかけて、あかりのことをただの幼なじみだと言ってやれば、それで解決だ。",
    "It will be easy enough to clear up the misunderstanding by going after her and explaining that Akari is just a childhood friend."),
    # If you immediately chase Akari and say that she's just a childhood friend, then it's time to smoke.

    ("けど…。",
    "But…"),
    # But...

    ("ただの幼なじみか…。",
    "Just a childhood friend, huh?"),
    # Just a childhood friend...

    ("「あかり」",
    "Akari."),
    # "Akari"

    ("「うん？」",
    "Yeah?"),
    # "Yeah?"

    ("「お前、オレの幼なじみか？」",
    "Are we childhood friends?"),
    # "Are you my childhood friend?"

    ("「えっ？",
    "Huh?"),
    ("うん、そうでしょ？」",
    "Yes, but why do you ask?"),
    # "Huh? Yes, that's right, slut?"

    ("「ただの幼なじみか？」",
    "Just childhood friends?"),
    # "Are you just a childhood friend?"

    ("「ただの…？」",
    "Just…?"),
    # "Just...?"

    ("あかりはうつむいて、少し考えると、",
    "She lowers her eyes and thinks for a while."),
    # Akari turns her head down, and after thinking about it for a moment,

    ("「どういうこと？」",
    "What do you mean?"),
    # "What does that mean?"

    ("複雑な表情を見せた。",
    "I can see she has mixed feelings."),
    # He showed a complicated table.

    ("「だから、わざわざ追っかけて訂正することもないって言ってんだよ」",
    "I'm saying there's no need to go out of the way to run after her to explain."),
    # "That's why I say there's no need to go along and revise it."

    ("「………」",
    "………"),
    # "..."

    ("深く取るもよし、軽く受け止めるもよし。",
    "I am okay with however Akari chooses to interpret that."),
    # You can also take a deep look, or take a light look.

    ("あーあ、なに言ってんだか。",
    "Oh boy… What am I even saying?"),
    # Ah, what are you talking about?

    ("眠い目が覚めちまったぜ。",
    "My sleepiness has been completely blown away."),
    # I'm awake sleepy.◯

]