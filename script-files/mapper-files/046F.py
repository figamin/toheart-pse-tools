# Generated array of Japanese text with English comments
# Source file: 046F.txt

text_array = [
    ("（志保	回秤数イベント	ＰＳ）閨（選択綜	ポ泣ルチェ）†",
    ""),
    # (Shiho Scales Event PS) (Selection So Pokryu Ruche)†

    ("「ポ泣ルチェ、か？」",
    ""),
    # "Porcelain, is it?"

    ("「え？	ポ泣ルチェ？	カッコいいけど、ＲＲだしぃ」†",
    ""),
    # "Huh? Sneaking Luce? He's cool, but he's RR" †

    ("「なんだよ、その、ＲＲって」",
    ""),
    # "What is that? RR?"

    ("「あら、知迺らないの？",
    ""),
    # "Oh, don't you know?

    ("リアエンジン・梶リアド宴ライブの略譌よ」",
    ""),
    # A shorthand for rear engine Kaji Riad Banquet Live."

    ("「知髓るかそんな烽もん。オ激レはマニアじ痰ゃねえ」†",
    ""),
    # "I wonder if that's the kind of hot thing. I'm a fan of you, so I have no phlegm."

    ("「っとに…、男のくせにさ」",
    ""),
    # "Ahhhh... I'm a man."

    ("「悪かったな」†",
    ""),
    # "That was bad" †

    ("「…なんなら教えよっか？	ＲＲの秘密を」",
    ""),
    # "...Could you tell me what? The secret of RR."

    ("てっきり『うんちく』を聞かされるのかと思ったら、意味深なことを言甯われ、少しだけオ激レは興味を揩持った。",
    ""),
    # Just as I thought he would be hearing "poo" something, he was told something meaningful and I was a little interested in it.

    ("「…ポ泣ルチェに秘密があんのか？」†",
    ""),
    # "...Is there a secret to Porcelain?"

    ("「それは『お尻』諱よ」",
    ""),
    # "That's the 'butt' name."

    ("「なぬ？	ケツ？	誰の？」†",
    ""),
    # "What? Butt? Who's?" †

    ("「アホ、ポ泣ルチェのよ」",
    ""),
    # "Idiot, it's a cry for you."

    ("「ポ泣ルチェのケツって、縺後撃のことだｂ」†",
    ""),
    # "Pokryu Rutche's ass is a hit after the twitch b"

    ("「ケツを振髏るのよ」",
    ""),
    # "Spin your ass."

    ("「ケツを振髏る〜っ？	お前、オ激レをペテンにハ＜メ諠ようとしてねえか？」",
    ""),
    # "You're shaking my ass? You're trying to cheat on me?"

    ("いい加減なことを≠吹き桙込んで、あとで笑い烽ものにするのは志保の得意技だ。†",
    ""),
    # Shiho's specialty is to do things casually and then make them laugh later.†

    ("すると『瘁違う墲わよ』と手のひらを横につきだして、",
    ""),
    # Then he said, "It's a different roar," and pointed his palm to the side,

    ("「いい？	カーブでアクセ泣ル踏むと、こう、梶リアタイアがス潟リップして｢｢」",
    ""),
    # "Is that okay? When I step on the accessory cry on the curve, Kaji Riataire said, ""

    ("と、ケツの重いポ泣ルチェがカーブでいかに不安闊定かをみっちりと聞かされた。†",
    ""),
    # He was thoroughly informed of how uneasy Luche, who had a heavy ass, was on the curve.†

    ("「…あのな、それは秘密で烽もなんで烽もねえよ。ポ泣ルチェの№欠点だｂ」",
    ""),
    # "...Um, that's a secret, it's not a hot spring or hot spring. It's a No. 1 flaw in Pokryu Rutche."

    ("「だから秘密だったんだけど、いまはすごく乗闖り竄やすくなったみたいね」",
    ""),
    # "That's why it was a secret, but it seems like it's really easy to tamper with."

    ("「いまは良くなったんだｂ？	昔のことまで隠してどうすんだ」",
    ""),
    # "Is it better now? What are you doing to hide the old days?"

    ("「それ烽もそうね…」†",
    ""),
    # "That's the case with Hot..."

    ("「自分で納得するな」",
    ""),
    # "Don't be convinced by yourself."

    ("「で烽もさで烽もさ、カ泣ル宴ラ４っていうのは｢｢」",
    ""),
    # "Then it's hot and hot, the hot and hot, and the name "Crying Feast 4" is ""

    ("「…增わかった墲わかった。お前が車好きなのはよーく墲わかった。小藷しい話しは他で竄やってくれ」",
    ""),
    # "...I understand, I understand. I understand that you love cars. Please tamper with the fake stories elsewhere."

    ("オ激レは志保に背をｌけた。†",
    ""),
    # I turned my back against Shiho.†

    ("「あ〜ん、最纃後まで聞いてよ〜〜〜っ」◯",
    ""),
    # "Ahhh, listen to me until the very end..." ◯

]