...
# Source file: 0543.txt

text_array = [
    ("（葵	好感度イベント２）閨（発生条緒件	好感度１５以繹上）閨（土曜以外の汲昼休み）閨（教室）†",
    ""),
    ...
    ("弁抹当を食べるため、あちこちで№仲間同士で机の移動が始まった。†",
    ""),
    # To eat the bento, desks began to move around with friends.†
    ...
    ("「あ、ごめん、。僕、今厓日弁抹当なんだ」",
    ""),
    # "Ah, sorry. I'm currently taking the Nikkei Bento."

    ("♂史は机の横に引っかけたバックから、袋に入った弁抹当箱を取闔り出した。†",
    ""),
    # Masochi picked up the bento box in a bag from the bag that had been hooked to the side of the desk.†

    ("「なんだ、またかよ？	最近多いな」",
    ""),
    # "What, come again? There's a lot lately."

    ("「うん、姉さん、お腹に赤ち痰ゃんできてから、ずっとヒマしてるから」",
    ""),
    ...
    ("氏去年渠結婚したｉ史の姉さん、いまはオ＜メデタヲ迺らしい。†",
    ""),
    # Mr. Ishi's older sister, who married last year, seems to be a fan of meditation.†

    ("「メ買いに行くだけで烽もつき№合おうか？」",
    ""),
    # "Should I just go and buy some hot water and get some good one?"

    ("「いいよ。ガキじ痰ゃねーんだから」",
    ""),
    # "That's fine. There's no phlegm."
    ...
    ("ばったり、葵ち痰ゃんと出初った。",
    ""),
    # I ran into it and first appeared with Aoi Chisho-chan.

    ("片阨手にバッグを揩持っている。†",
    ""),
    # He holds a bag in his hand.†
    ...
    ("「あ、あの、この前、約束したので、だからр私、″ってきたんですけど…」",
    ""),
    # "Ah, um, I made a promise the other day, so that's why I came here..."
    ...
    ("閨手詩闕り弁抹当だって！？†",
    ""),
    # It's a hand-picked piece of bento!?†

    ("「いえ〜〜〜〜いっ！	轣らっきぃ〜〜〜〜、そいつは嬉しいぜ〜〜〜〜〜〜〜〜っ！！」",
    ""),
    ...
    ("「せ、諱先輩。あの、恥ずかしいですから…」",
    ""),
    # "See, Senpai. Um, it's embarrassing..."
    ...
    ("「諱よし、それじ痰ゃさっそく、そいつをごちになるとしようか。屋繪上で烽も行こうか？」",
    ""),
    # "Okay, let's just go and give him that one right away. Shall we go to the shrine at home?"
    ...
    ("「う墲わぁ、いい天気ですねっ！」",
    ""),
    # "Uhh, the weather is nice!"

    ("陽の光を≠浴びながら、うーんと背伸びした。†",
    ""),
    # I stretched out, groaning as I was exposed to the sunlight.†
    ...
    ("葵ち痰ゃんが烽金網の下を指さして言った。",
    ""),
    # Aoi Chisho-yan pointed under the wire mesh and said.
    ...
    ("葵ち痰ゃんは感嘆の息を漏逖らした。",
    ""),
    # Aoi Chisho-yan let out a breath of admiration.
    ...
    ("「最近の映謇画とかは、見てないのか？」",
    ""),
    # "Did you not look at recent film paintings?"

    ("「…ええ、そういうのはあまり」",
    ""),
    # "...Yeah, that's not that."

    ("「最近じ痰ゃあ、そういうのが流行闕りなんだよ。",
    ""),
    ...
    ("それにして烽も、天気は最ｍ高、景色熕もいいし、おまけに気搴持ちのいい翌風まで≠吹いている。",
    ""),
    # However, the weather is at the highest point, the scenery is nice, and even the next wind is blowing well, even when it's still good for good.

    ("こんな最ｍ高のセッティングで、葵ち痰ゃんの手料摎理が食べられるとはな。",
    ""),
    # I don't know that Aoi Chisho-yan's handling fees can be eaten with such a high setting.

    ("♂史のрヤツ焜も、ホント、いいタイミングで弁抹当を揩持ってきてくれたぜ。†",
    ""),
    # The guy from Masochist also really brought me a bento at the right time.†

    ("葵ち痰ゃんはバッグから、可愛い袋に入ったお弁抹当を取闔り出した。",
    ""),
    # Aoi Chisho-yan picked up a bento box in a cute bag from her bag.
    ...
    ("お弁抹当のｒ身は、オーソドックスだが盛關りだくさんな烽内容だった。",
    ""),
    ...
    ("「あ、いちおう、それ烽も…」",
    ""),
    # "Ah, that's the hot water too..."

    ("「閨手詩闕り？」",
    ""),
    # "Are you talking about your hand?"

    ("「はい」",
    ""),
    # "yes"

    ("「凝ってる凝ってる、十分だよ。ウインナーだって、ほら、ち痰ゃんとタコさんだし」†",
    ""),
    # "It's so clumsy, it's enough. Even the wieners are, you see, sputum and octopus."

    ("「で烽も、正直に言うと、お母さんに手伝って烽もらったのがほとんどなんです」",
    ""),
    ...
    ("ハンバーグ窿やタコウインナーが、そんな昔からあるのかというツッコミはおいとこう。†",
    ""),
    # Let's leave a comment about whether hamburger steak and tako wielding have been around for such a long time.†

    ("「それじ痰ゃあ、いっただっきまーす！」",
    ""),
    # "That's all, I'll give you a phlegm!"

    ("じ痰ゃあまずは、この手詩闕りハンバーグからいただこうか。",
    ""),
    # First off, let's start with this manual hamburger steak.
    ...
    ("緊張し、不安そうな顔で見つめる葵ち痰ゃん。",
    ""),
    # Aoi Chisho-yan looks nervous and worried.
    ...
    ("「ウマイっ！	うん、いけるよ、これ！」",
    ""),
    # "It's tasty! Yes, I can do it, this!"

    ("「ほ、本末当ですか！？」",
    ""),
    # "Huh, are you the truth?!"
    ...
    ("お世辞じ痰ゃない、本末当にうまかった。†",
    ""),
    # There was no flattering phlegm, it was delicious in the end.†
    ...
    ("食べ終尞わった繧後、弁抹当箱を片づけながら、葵ち痰ゃんが言った。",
    ""),
    # After finishing the meal, Aoi Chisho-yan said as he cleaned up the bento box.

    ("「諱先輩がこんなに烽もｊんでくれるなんて…。本末当、″ってきてよかったです！」",
    ""),
    # "I can't believe that Senpai Koto will be giving me such a hot spring...I'm glad that I've been there for the sake of the day!”

    ("そして、にっこり笑った。",
    ""),
    # And then he smiled.

    ("その笑迴顔を見たとき、なんだか胸のあたりがドキッとなった。◯",
    ""),
    # When I saw that laughing face, my heart started to throb.◯
