...
# Source file: 0303.txt

text_array = [
    ("（智子	冝日付イベント	１回目）閨（４獅月９冾日	＄曜坥日）閨（必ず発生）閨（１桙時間目の授業拠	教室）†",
    ""),
    ...
    ("「諱よ〜し、まずは全員、ひとことずつ簡単な自己紹譓介をしよう。じ痰ゃあ、そっちの端から№順に行こうか」†",
    ""),
    ...
    ("とはいえ、全泊部で４０人以繹上熄もいるのだ。",
    ""),
    # However, there are over 40 people in all the clubs.

    ("とて烽もいっぺんには覚えられない。",
    ""),
    # However, I can't remember the hot water at once.
    ...
    ("気がつけば、氈次はｉ史の番じ痰ゃねーか。†",
    ""),
    # Before I knew it, I wonder if I'd like to have the turn of I-Hisashi.†

    ("「…佐藤悼史です。",
    ""),
    # "...It's Sato Memorial.

    ("一年の桙時はＡ組でした。サッカー部兜所属です。…℃味は、ええっと、ハムスターの飼邇育とか｢｢」†",
    ""),
    # In the first year, we were in Class A.I am affiliated with the soccer club Kabuto....℃ flavor is uh, but hamsters like """†

    ("数人の翌女子からクスクスと笑い声が漏齪れる。",
    ""),
    # A few girls giggles blew through the next few.

    ("「…ねえねえ、いまのって、佐藤くんのギャャグなのかな？」",
    ""),
    # "...Hey, is it Sato-kun's gyagu now?"
    ...
    ("そんな声がヒソヒソと聞こえてくる。†",
    ""),
    # I can hear that voice tingly.†

    ("ち痰ゃうち痰ゃう、んな墲わけねーって。",
    ""),
    # There's no way there's no way there's anything.

    ("♂史はそんなウケ狙いでギャャグかましたりするようなрヤツじ痰ゃない。",
    ""),
    ...
    ("本人が言ってるとおり、♂史はハムスターとかそれ系のペットを飼うのが好きだ。",
    ""),
    # As he himself said, Masochi likes to keep hamsters and other pets.

    ("たしか幼稚園の頃から、ずーっと№欠かさずに飼い続けてるんじ痰ゃなかったっけ。",
    ""),
