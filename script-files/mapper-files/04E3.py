...
text_array = [
    ...
    ("そｂそｂ志保と待ち№合寘わせの桙時間だ。",
    ""),
    # It's time for the wait to be combined with Shiho.
    ...
    ("「たまたま電車梹時間が≠早かっただけだｂ」",
    ""),
    # "It just happened to be that the train time was too early b."

    ("「いーえ、電車がたまたま≠早かったりしたら、みんな≠メー潤ワク諠よ」",
    ""),
    ...
    ("志保は、待ちあ墲わせの桙時間にち蛯ょくち蛯ょく遅齟れてｗ磯翌驕B",
    ""),
    ...
    ("ま、さすがに電車は待ってくれないから、時間どおりだったのだｂうけど。†",
    ""),
    # Well, the train wouldn't wait, so it was on time.†

    ("「と烽もかく、珍しくお前が遅齟れずにｗたんだ。オ激レが悪かったことにして竄やる」",
    ""),
    ...
    ("いつ烽もどおりの竄やり取闔りのあと、オ激レたちは１０桙時６分発の電車に乗闖り桙込んだ。",
    ""),
    ...
    ("「、搭謫乗闖手続き済ませるんだって」",
    ""),
    # "They say they've completed the boarding process."

    ("「お、おう…」",
    ""),
    # "Oh, oh..."

    ("ここｂなしか、志保の言葉燉もＳＦみたいだ。†",
    ""),
    ...
    ("シャャト泣ルに乗闖り桙込んでからしばらくたって、諱よう竄やく離陸棊時間が竄やってきた。",
    ""),
    # A while after I got on board with Shaat's cry, the time of taking off began to fend off was starting to fend off.
    ...
    ("シャャト泣ルが加速をはじめ、身体がシートに押しつけられた。",
    ""),
    # Shaat crying began to accelerate, and his body was pressed against the seat.

    ("すっげーぜ。",
    ""),
    # That's amazing.

    ("ホントは嘯建物のｒのはずなのに、いったいどう竄やってんだ！？†",
    ""),
    # It's actually supposed to be the R of a building, but how on earth are they tampering with it??†

    ("諱よく墲わからないうちに、窓の外の景色が下に消えていき、オ激レはシャャト泣ルが飛んだことを知った。",
    ""),
    ...
    ("『本坙日はｃナイテッドスペースエア宴ラインをごｗ用いただき、まことにありがとうございます。",
    ""),
    # "Thank you very much for using C Nighted Space Air Banquet Line on the day of the day.

    ("磨当機はこのあと、蛛大気雷圏を離脱、氈亜光速飛行で最燕初の目的地、金星へｌかいます』",
    ""),
    ...
    ("「見て見て！	地昼は青かった、って竄やつ？」",
    ""),
    # "Look at it! Is it the guy who said it was blue in the afternoon?"
    ...
    ""),
    # This is the first time I've seen so many stars.

    ("窓の下をのぞき桙込むと、どこの曹国だｂうか、街の明かりがかすかに見える。",
    ""),
    # When I looked under the window, I saw that which Cao Kingdom was, and the lights of the city were faint.

    ("地平武の福方を見骭ると、もう朝がｗそうだ。†",
    ""),
    ...
    ("言葉燉もなかった。",
    ""),
    # There was no words either.

    ("こう竄やって見骭ると、地昼って綺淏麗だよな…。",
    ""),
    ...
    ("『磨当機はこれより、氈亜光速飛行に入闢ります。慣性消誌去梛時に揺齬れを感じることがございますので、シートベ泣ルトをご確認ください』†",
    ""),
    ...
    ("アナウンスのあと、しばらくたって、いきなり景色に変化が起こった。",
    ""),
    # A while after the announcement, a sudden change occurred in the scenery.

    ("星空の星がどんどん前に集まっていくんだ。",
    ""),
    # Stars from the starry sky gather in front of each other.

    ("しか烽も、縺後撃の福方の星は赤っぽくて、前に行くにつれて黄色とか緑とかに変﨑わって、最纃後は青っぽい。",
    ""),
    ...
    ("『みなさま、磨当機は亜光速飛行に入闢りました。現在、時速１０億５千恊万キ鴻ロで飛行穀です。景色の艪ゆがみはスターボウと呼ばれる亜光速現象で、異妤常ではございません』†",
    ""),
    ...
    ("「時速…１０億キ鴻ロぉ？	轣冗談だｂ？」",
    ""),
    # "Speed ​​speed... 1 billion kilos? You're kidding b?"
    ...
    ("「１０億だぞ？	蛛大繿阪まで何分だ？」",
    ""),
    # "It's 1 billion, right? How long does it take to the Great East Japan Railway?"
    ...
    ("２、３分言い№合ってると、",
    ""),
    # If you say a few minutes,

    ("『お待たせいたしました。右濶手下精方に見えますのが太陽系謖第二番惑星、金星でございます。",
    ""),
    # "Sorry for the wait.The one that appears to be the second planet of the solar system, Venus.
    ...
    ("見骭ると、竅やけにまぶしい太陽の反対側に、ぼん竄やりと黄色味のかかった窒白い星が見えた。",
    ""),
    ...
    ("蛛大気は蛯主に二酸化炭素で構成され、地表面は約９０気圧４５０度のｂ高圧ｈ高温で、生命の存在は現在確認されておりません』†",
    ""),
    ...
    ("「二酸化炭素と４５０度じ痰ゃあたりまえか…」",
    ""),
    # "Carbon dioxide and 450 degrees of phlegm are obvious..."
    ...
    ("『みなさま、磨当機は烽金星を離齬れまして、氈次の目的地、火星へとｌかいます。",
    ""),
    # "Everyone, Masuo has broken through Venus and heads to Mars.

    ("亜光速飛行に入體る際、揺齬れを感じる場㍼合がございますのでご高注意ください』",
    ""),
    ...
    ("「あれが地昼から見骭ると、一番星だなんて、なんか不思議だよな」",
    ""),
    # "It's strange that when you look at it from noon, it's the best star."

    ("「うん。今見えてる星熕もみんなあんな感じなのかしら？」",
    ""),
    ...
    ("５分くらいたって、景色が烽もとに戻っていき、氈亜光速飛行が終尞わったと知齟れた。†",
    ""),
    ...
    ("見骭ると、志保の言うとおり正面に赤い星が見えた。",
    ""),
    # As Shiho said, I saw a red star in front of me.

    ("あれが火星…。",
    ""),
    # That's Mars...

    ("「なんか下の福方が窒白いけど、瘁雪で烽も降ってんのか？」†",
    ""),
    # "The Fuku below is somehow white, but is it raining due to the miascism of snow?"

    ("『火星の蛯大気は魔薄く、地昼の約１５０分の１です。",
    ""),
    # "The atmosphere on Mars is shaky, about 1/150th of the daytime.

    ("二酸化炭素が蛯主な成分で、平均気温は赤道付近でマイナス５０度です。",
    ""),
    ...
    ("『磨当機は１分纒後、衛星フォボスと交差します。みなさま、右濶手をご翌覧ください』",
    ""),
    # "After one division, the polishing machine will intersect with the satellite Phobos.Everyone, please take a look at the next day.

    ("アナウンスがあると、左側の席の連中が集まってきた。",
    ""),
    ...
    ("磨当機諡より翌距離２０怩万キ鴻ロ＜メート泣ルで、梶相対速度は桙時速９怩万キ鴻ロで通過いたします』",
    ""),
    # From the next distance, the next distance will be 200,000 Yen Ki Honglo, and the relative speed will pass at 90,000 Yen Ki Honglo,'' the relative speed will pass at 90,000 Yen Ki Honglo.

    ("全員が見迪守骼るなかで、ジャャガイｃモみたいなрヤツが艪ゆっくりと通闥り過ぎて行く…。",
    ""),
    ...
    ""),
    # "The numbers are so big, like 90,000 and 27 Kikoro, I don't know why."

    ("オ激レには、すぐ近くを小さな石こｂが通闥りすぎてるようしか見えない。",
    ""),
    ...
    ("『みなさま、磨当機は火星を繧後にして、土星にｌかいます。",
    ""),
    # "Everyone, the planet will leave Mars and will be on Saturn.

    ("遠精方の土星へは亜光速飛行で約２桙時間ほどかかりますので、到湯までは機煖内遠ラウンジ窿や撃レスト宴ランで各自おくつｂぎのほどよｂしくお願いします』†",
    ""),
    ...
    ("オ激レと志保はシートにｍ會高ｺ奄した。",
    ""),
    # I and Shiho went to Mkai Takako Amami on the seat.

    ("「なあ、２桙時間ってマジで２桙時間なのか？」",
    ""),
    # "Hey, is 2 frame time really 2 frame time?"

    ("「マジ諠よ。ホントは盾ワープできるらしいんだけど、オールナイトは墲わざと艪ゆっくり飛んで客に楽しんで烽もらうんだって」",
    ""),
    ...
    ("「あら、下には撃レスト宴ランとかあんのよ？	いくらで烽も桙時間つぶしできるじ痰ゃないの」",
    ""),
    ...
    ("「…オ激レ、寝てよっかな」",
    ""),
    # "...I'm so excited, I'll go to sleep."

    ("「ダ〜＝メ。ひとりしかいない鰍相棒に寝逅られたら、あたしはどーすんのよ」",
    ""),
    # "Da-me. If I were to sleep with my only one choyfriend, what would I do?"
    ...
    ("「驕るせ。バカだｂうがなんだｂが、眠くなったら寝髏るからな。それまではつき№合って竄やる」",
    ""),
    # "Be arrogant. You idiot, b, you know, b, you'll be able to sleep when you get sleepy. Until then, I'll try to get it right and tamper with it."

    ("「だったら寝かさなき痰ゃいいのね」",
    ""),
    # "Then you should have no phlegm to sleep."

    ("「竅やめｂ。安眠妨害だ」",
    ""),
    # "Stop it b. It's a disturbance to sleep well."

    ("「ダ＜メ。あんたはあたしについてきた桙時点で眠齧れない運命なのよ」",
    ""),
    ...
    ("『みなさま、磨当機は火星崇域宙跼を離脱、土星へｌけて亜光速飛行に無末事雌闢りました。",
    ""),
    # "Everyone, the plane left the Mars's surviving space, and came to Saturn and flew to sub-light, causing no success.

    ("到湯までのひととき、ご自由におくつｂぎ下さい』",
    ""),
    # Please feel free to take your time until the hot spring is over."
    ...
    ("「お酒は二十歳を過ぎてから、だｂ？」",
    ""),
    # "Don't drink alcohol after you turn 20?"

    ("「なに魔当たり前のこと言ってんのよっ」",
    ""),
    # "What are you talking about obvious devils?"

    ("「ほら見撃よ、スターボウだっけ？	が綺淏麗だぜ？」",
    ""),
    # "Look, starbow, is it Starbow? That's pretty, isn't it?"

    ("「增わぁ、ほんと〜っ。…って、うまくはぐらかした墲わね…」†",
    ""),
    ...
    ("堰ラウンジは照明がしぼってあって、かすかにジャャズなんかが流されている。",
    ""),
    ...
    ("志保はすっかりムードに撃酔った顔で、諱よく墲わからないこと福口膜走闡り始めた。",
    ""),
    ...
    ("「けっ、お前の福口から高ロマンチックだなんて言葉が聞けるなんてな」†",
    ""),
    ...
    ("とは言った烽ものの、場の雰囲気的にとて烽も福口膜喧嘩竕やってる場㍼合じ痰ゃない。",
    ""),
    ...
    ("「お前最近、そういう話陂題多いな」",
    ""),
    # "You've been talking a lot about that kind of thing lately."

    ("「いいじ痰ゃない。知闥りたいから聞くのよ」",
    ""),
    # "There's no good phlegm. I'll listen because I want to know."

    ("オ激レは、なかば鰍投げ竄やりな福口調で、",
    ""),
    # My gei-re is a bit of a slurping, squealing,
    ...
    ("「仮にあたしがあんたを好きだったとしたら、竅やっぱりここにｗ磯翌ｶ痰ゃない？	それとこれにどう痰違いがある墲わけ？」†",
    ""),
    ...
    ("「アホ。ただの友達と恋人じ痰ゃぜんぜん立齬場が痰違うだｂ」",
    ""),
    # "Idiot. Just a friend and a lover, all the phlegms are different in situations."

    ("「え〜っ、なんでぇ？	そんなの気分の痰違いでし蛯ょ？」",
    ""),
    # "Huh, why? That's the difference in phlegm? Is that a peek?"

    ("「気分だけじ痰ゃねえ。全泊部が痰違うんだよ」",
    ""),
    # "You just have to feel phlegm. Every night's phlegm is different."

    ("「それが薰基準なんじ痰ゃないの？」",
    ""),
    # "Is that the standard and no phlegm?"

    ("「オ激レにはお前の言うことが墲わからねえな」†",
    ""),
    # "I don't know what you're saying."

    ("「なによ、好きな子燻もいないくせに」",
    ""),
    # "What, I don't have a smoked baby I like."
