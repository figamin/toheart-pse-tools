...
text_array = [
    ("（ゲーセンバト泣ル１）閨（卒業式篇）閨（選択綜	志保）閨（蛛主人公煙負け）閨（ゲーセン焜内）†",
    ""),
    ...
    ("ちらりと繧後撃を見骭ると、はち切齔れんばかりの笑迴顔の志保が…。†",
    ""),
    ...
    ("「…敗者はどうなる運命か、增わかってる墲わよね？」",
    ""),
    # "...You know what the loser's fate is, right?"

    ("「…氏虫葡歯をなおすんだｂ」",
    ""),
    # "...I'll heal my paper bugs and grapes."

    ("「き痰ゃははっ、おっ烽もしｂ〜い。そんなこと言って烽もあんたの運命は変﨑わらない墲わよ〜っ」†",
    ""),
    ...
    ("内容的には汲結構いい勝縁負だったんだよな。†",
    ""),
    # In terms of content, I was pretty good at winning.†

    ("オ激レのほうは、志保にрヤックをおごる羽目になった。",
    ""),
    # My guts ended up salking Shiho for a yuck.

    ("「あたし、スト鴻ロベ潟リーパイ１個だけ〜」",
    ""),
    # "I'm only one Stoko Robegata Ree Pie."

    ("「おう、安い烽もんだ」",
    ""),
    # "Oh, it's cheap hot water."

    ("「…と思寬わせて、カ泣ルテットсヤック焜も１個！」",
    ""),
    ...
    ("本編で志保のスコアが抜けない福方へ、アドバイス。",
    ""),
    # Here are some advice for Fukukata, who can't get past Shiho's score in the main story.

    ("あのスコアを抜くには、１面でのスコア稼ぎが必要です。",
    ""),
    # To surpass that score, you need to earn a score on the first level.

    ("ポイントは二つ。",
    ""),
    # There are two points.

    ("１５０ＨＩＴ以繹上出すことと、青以外のアイテムを取邇らないことです。",
    ""),
    ...
    ("１面は２箇渇所で赤シ㏍ョットに切關り換える以外は青シ㏍ョットで進みます。",
    ""),
    # On the first side, use the blue one except to switch to a red one at two dry places.

    ("それを踏まえた繧上で、まずは一つ目のポイントである１５０ＨＩＴから。†",
    ""),
    # Taking this into consideration, we'll start with the first point, 150 HIT.†

    ("コンボ開始は１面が始まってしばらくたって、戟レーザーさん浮＆クマッキー×２が登齠場する場面の手前、緑耐性で画面外且をぐるぐる回驩る「人形さん緑」×５が２グ泣ループと「お化け」×３からスタートです。（志保のコンボパターンは古いので、開始地点が遅くなっています）†",
    ""),
    ...
    ("人形さんをｂ撃たずに待ち、お化けをギ潟リギ潟リまで引きつけてからコンボ開始です。",
    ""),
    ...
    ("途湯、青シ㏍ョット耐性を揩持つ「マナ」が出現するなど厳しい場鰹所がありますが、無許視して伸びウサギでつないでいけば１５７ＨＩＴ前繿後まで出ます。志保のプ激レイでは、縺上から降ってくる「氏落下さん」を逃がしているのでＨＩＴが獅落ちています。",
    ""),
    ...
    ("撃ち烽もらし竄やすいので高注意しまし蛯ょう。†",
    ""),
    # Be careful as it is easy to fire and fire.†

    ("戟レーザーさん浮＆クマッキーは、戟レーザーさんを倒してから、墜賜落するクマッキーを倒すことで１０倍のボーナス点が入闢ります。その繧上コンボの倍率燉も繧上謠乗せされるので、１５５ＨＩＴ目で出せば一気に１２００怩万点以繹上藷闢ります。コンボをつなぎながら狙える個褐所が二つあるので（志保プ激レイ参照）、狙ってみまし蛯ょう。†",
    ""),
    ...
    ("シ㏍ョットアイテムは同色を取闔りつづけることで、５個目から１０怩万点です。これにコンボの倍率がかかることで重要な得点源となります。（ちなみに、１５７ＨＩＴ継続葬に取骼ることで１個４８１怩万点になります）†",
    ""),
    ...
    ("以繹上のポイントを守齊れば、１面ク潟リアで３３００怩万点以繹上出ます。この桙時点でオールク潟リア档時のスコアが志保を繧上回驩るのは間瘧違いありません。",
    ""),
    # If you follow the above points, you will be able to get over 3,300,000 points on the Kugata Ria.At this point, it is undoubtedly difficult to see that the score at the Olkugata Rear Frame exceeds Shiho's.

    ("閨『コンボの公式』",
    ""),
    # "Combo Official"

    ("取得点×（１＋（ＨＩＴ−１）×０．３）がスコアにプ宴ラスされます。†",
    ""),
    ...
    ("開糾幕からボス前までコンボがつながります。",
    ""),
    # Combos connect from the opening curtain to the boss.

    ("しかしタイミングが異妤常にシビアで、人間業では安闊定は不可能に思えます。",
    ""),
    # However, the timing is always harsh, and it seems impossible to be able to be accustomed by humanity.

    ("実際は伸びウサギからスタートして、縺後撃から現鼬れる「п槍タイツさん」×４の直前までの７６ＨＩＴが実用的でし蛯ょう。†",
    ""),
    ...
    ("とりあえず５０ＨＩＴ以繹上を目指しつつ、道湯は緑シ㏍ョットを撃レベ泣ルアップさせ、以繹後は青と緑だけを狙って取闔りまし蛯ょう。",
    ""),
    # For now, while aiming for the 50th birthday, I fired a green shout at Doyu and moved up to tears, and after that, I tried to aim only for blue and green.

    ("ボスの「增戦艦ミ激レニアム」では砲莓台からの弾をひたすらイチゴにしてスコアを稼ぎます。",
    ""),
    ...
    ("逃げる前に倒すのですが、途湯の蛯大型砲莓台でジャースを回精収するのを忘齧れずに。†",
    ""),
    ...
    ("ですがコンボパターンは簡単にｌまる烽ものではありません。敵のｖ鰯哩ﾏ化する攻ｍ撃タイミングに№合寘わせてアド潟リブでパターンを竄補正する必要があります。†",
    ""),
    ...
    ("パターンを安闊定させるには、何度燗もプ激レイしてより多くの状況を経験することです。アド潟リブのパターンフォ鴻ローが充実するでし蛯ょう。",
    ""),
    # To make the pattern stand out easily, you need to heat it up and experience more situations.The pattern of Adgata Libs is packed with plenty of patterns.

    ("粘阡り強くプ激レイして、気分謨よく志保を竄やっつけてあげまし蛯ょう。◯",
    ""),
    # He thrusts hard and violently, and gives Shiho a strong feeling and gives him a bluff.◯

    ("本編で志保のスコアが抜けない福方へ、アドバイス２。",
    ""),
    # Here are 2 advice for Fukukata, who can't get past Shiho's score in the main story.

    ("どうして烽も志保に勝てない福方はノーシ㏍ョットボーナスを狙ってみまし蛯ょう。",
    ""),
    # Why is Fukukata unable to beat Shiho? Let's aim for a no-stop bonus.

    ("一度燗もシ㏍ョットを打たず、ボムだけでク潟リアすると７０００怩万点涛闢ります。",
    ""),
    # Once you've cooked it, you can get 7,000,000 points when you don't play the ball with just the bomb.

    ("ただし、クマまではノーミスで行って『彼濫女』を出さなくてはなりません。†",
    ""),
    # However, you must make no mistakes to the bear and make a ``boyfriend'' girl.†

    ("ポイントは、アイテムキャャ潟リアーで確実にボムを取闔りつづける魔事と、ボム黹一発では倒せない「なまハム宴ライダー」の攻ｍ撃を、氈次のアイテムキャャ潟リアーがｗ磯翌ﾜで耐えしのぐことです。†",
    ""),
    ...
    ("また、ボスは倒せないので逃げていくまで根気諡よく攻ｍ撃をか墲わしてください。",
    ""),
    # Also, you cannot defeat the boss, so please take a patient attack and attack until you run away.

    ("『彼濫女』が出た繧後は、ミスして烽もかま墲わないので、残っているボムを有効に使って下さい。◯",
    ""),
    # After "Hibana" is released, you make a mistake and won't get enough of the hot water, so please make effective use of the remaining bombs.◯
