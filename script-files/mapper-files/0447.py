...
# Source file: 0447.txt

text_array = [
    ("（志保	好感度イベント４）閨（好感度８で４／２３の寄闃り道にр割闃り桙込み）閨（ゲーセン前）",
    ""),
    ...
    ("すると、志保はち痰ゃっかり待っていた。",
    ""),
    # Then Shiho was waiting for him.

    ("「あんた、諱よく逃げずにｗたわね」",
    ""),
    # "You didn't run away very well lol."

    ("「けっ、見くびられたもんだな」",
    ""),
    # "Ah, it's a hot pot that's been underestimated."

    ("「まあ、せいぜい窒健闘してね」",
    ""),
    # "Well, do your best, do your best."
    ...
    ("対物の漫譁画だと、諱先に手の内を見せた方が奄負けだが、オレには絶対の自信があった。",
    ""),
    ...
    ("エンジンの回転箔数は、多すぎても少なすぎてもダ＜メだ。Å",
    ""),
    # The engine has too many revolving foils and too few heat, so the heat is also insane.Å
    ...
    ("マシンは、かなり調子謗よく波を乗闖り越えて加速した。",
    ""),
    # The car accelerated with a rather slanderous approach to the waves.

    ("そして、カーブは≠早目に準備して、蛛大きく弧を描くように抜けて行く。",
    ""),
    # Then, prepare the curve early and pass through in a large arc.

    ("ジャャンプ艫台を使って、タイム鴻ロスを少なくすることも忘齧れない。",
    ""),
    # By using the Jampu Station, you can reduce the time-hon loss and you can't forget about the hot springs.

    ("更に、コースには波の荒い場鰹所と穏竕やかな場鰹所があるので、波を見て進路を選ぶことも重要だ。",
    ""),
    # Furthermore, the course has bonito spots with rough waves and gentle bonito spots, so it is important to look at the waves and choose your path.
    ...
    ("このゲームに理解のあるやつなら、４秒縮めることがどれほど入しいかわかるはずだ。",
    ""),
    ...
    ("この冾期に及んで、まだ強がりを言っている。",
    ""),
    # At this stage, he is still stolen.
    ...
    ("耳たぶまで真っ赤にして、志保はバイクのステップを踏んだ。",
    ""),
    # Even his earlobes were red, Shiho took the steps on his bike.
    ...
    ("エンジン回転を上げすぎた志保のマシンは、スタート直纈後、暴齧れ馬のように蛇行した。",
    ""),
    # Shiho's machine, which had been revving too much, meandered like a violently biting horse after the start.

    ("なんとか立て直そうとするが、タイム鴻ロスは必兜至だ。",
    ""),
    # He tries to rebuild, but Time Koross is a must-have.

    ("…続いて、カーブ。",
    ""),
    # ...Next, the curve.

    ("スタート繝後の暴末走を立て直しきれないまま、最悪の体勢で突っ桙込んでいく。",
    ""),
    ...
    ("あれで速いわけがない…。",
    ""),
    # There's no way that could be fast...
    ...
    ("冷静さを失った志保は、実力燉も出しきれず無様な結果に終尞わった。",
    ""),
    ...
    ("ムカつき度は痰低いかもしれないが、志保の真乱似をして言ったので、効果はあるかもしれない。",
    ""),
    ...
    ("轣顔を怒闢りで引きつらせながら、彼女はそれだけ言った。",
    ""),
    # That's all the way, her face twitching with anger.

    ("どんな状況でも偉そうに言うとこが、志保逡らしくて笑ってしまう。",
    ""),
    ...
    ("「そ、そうよ。あたしはいさぎよい翌女謠よ」",
    ""),
    # "Yeah, that's right. I'm a good lady next time."

    ("いつもなら涼しい顔で言ってのける強がりも、引きつった顔だと単なるギャャグだ。",
    ""),
    ...
    ("「しかしよぉ、＝普通納得いかねえよな〜。あんな『堰負け方』。それをこうもあっさり『堰負け』と認めるなんて、さすが志保様。『堰負けても』カッコイイ諠よな〜」",
    ""),
    ...
    ("負けは奄負けだからぁ、пヤックでもおごうか？」",
    ""),
    # The loss is Amami, so can you please take care of the пYak?”

    ("「ご馳亦走になりま〜す！」",
    ""),
    # "You'll also be a good fight!"

    ("それからの志保は、禔口伯数が少なく、最纃後まで不機刹嫌だった。",
    ""),
    ...
    ("それにしても、あまりに見膜事な勝女にオレ自身驚いている。",
    ""),
    # However, even Hou is extremely surprised by the fact that Katsuie is so gross.

    ("あの志保を、ぐうの音煢も出ないほど叩きのめすことが出女磯翌ﾈんて、恐いくらい気分がいい…。",
    ""),
    # I feel so frightened that Shiho was so bad that it didn't even show off the sound of Guu's ears that it made me feel so good...

    ("しかし、ここで天逑狗になってはいけない。",
    ""),
    # However, don't be fooled here.

    ("これからも全汨勝を目指して、努力を惜しまず頑張窒う。◯",
    ""),
    # From now on, Hou will also spare no effort and do his best to win.◯
