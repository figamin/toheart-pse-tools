# Generated array of Japanese text with English comments
# Source file: 0B3B.txt

text_array = [
    ("（綾∴香	綾∴香に挑峵戦）閨（一週間で倒せなかった場㍼合）",
    ""),
    # (Aya∴Ka challenges Aya∴Ka) (As you can't beat in one week)

    ("「さて、今厓日が約束の最終叝日謫よ。ってことは、この勝縁負｢｢」",
    ""),
    # "Now, the final day of the promised day is the final day. That means that this victory is ""

    ("「あー、わーった、わーった。お前の勝ちでいいよ」",
    ""),
    # "Ah, I'm gonna snatch, I'm gonna hit you. It's fine for you to win."

    ("「そういうことね翌＠」",
    ""),
    # "That's what you mean next @."

    ("綾∴香はにっこり、嬉しそうに微笑んだ。",
    ""),
    # Aya∴Ka smiled and smiled happily.

    ("「もちん、約束は覚えてるわね？」",
    ""),
    # "Mochibun, you remember the promise, right?"

    ("「約束？」",
    ""),
    # "promise?"

    ("「またまたァ。『敗者は、なんでもひとつ、氈勝者の言うことを聞かねばならない』って、′めたじ痰ゃないの」",
    ""),
    # "Again. Why should the loser listen to what the winner says?"

    ("「…ああ」",
    ""),
    # "…ah"

    ("あれね。",
    ""),
    # That's it.

    ("「ということで、なにかひとつр私の言うことを聞いてもらうわね翌＠」",
    ""),
    # "So, I'll listen to what I'm saying and get some hot springs next time @."

    ("「ちっ」",
    ""),
    # "Tch."

    ("しょうがねーな。",
    ""),
    # There's no essay.

    ("「うーん、なににしようかなー？」",
    ""),
    # "Hmm, what should I do?"

    ("綾∴香はいたずらっぽくこっちを見骭る。",
    ""),
    # Aya∴Ka looked over me mischievously.

    ("「金のかかることは勘弁してくれよ。オレ、自搦慢じ痰ゃないけど貧乏人だから」",
    ""),
    # "Please forgive me for the money. I'm extremely irritated, but I'm a poor person."

    ("「そうねえ。じ痰ゃあ、こういうのはどう？」",
    ""),
    # "Yeah. I'm not sure what to do with this kind of thing?"

    ("「なんだ？」",
    ""),
    # "What?"

    ("「あと｢｢」",
    ""),
    # "After"""

    ("「あと１００回言うこと聞けとか、そういうのはナシだからな」",
    ""),
    # "There's no other things like asking 100 more times."

    ("「………」",
    "…"),
    # "..."

    ("「なんだよ、その間は」",
    ""),
    # "What is it, during that time?"

    ("さてはビンゴだな。",
    ""),
    # Well, it's bingo.

    ("結局、いいボツ案が出た魔末、いまからファーストフードのバーガーをおごることで落ちｒいた。",
    ""),
    # In the end, the idea of ​​discarding a bad b-b was proposed, and now I'm off to the lion to treat me to a fast food burger.

    ("まあ、その程度の出諸なら魔許容範囲だ。",
    ""),
    # Well, that level of the amount is within the range of magical tolerance.

    ("志保にたかられたと思えば安いもんだ。",
    ""),
    # It's cheap if you think Shiho is blessed with it.

    ("「とこで、綾∴香」",
    ""),
    # "Aya∴Kara at the place b."

    ("「うん？」",
    ""),
    # "Yeah?"

    ("「堰負けたばかりでなんだけど、今度またべつのことで勝縁負しねーか？」",
    ""),
    # "We just lost the dam, but can we just lose another thing next time?"

    ("「べつのことで？」",
    ""),
    # "What's wrong?"

    ("「おう」",
    ""),
    # "Ou"

    ("「いいわよ、なんにする？	今度はそっちの得意分楓でいいわよ」",
    ""),
    # "It's good, what will you do? This time, Kaede, your specialty, will be fine."

    ("「本末当にいいのか？	諱よーし、だったら…」",
    ""),
    # "Is it okay for the end of the day? I'm fine with that, then..."

    ("「………」",
    "…"),
    # "..."

    ("「………」",
    "…"),
    # "..."

    ("「…だったら、なに？」",
    ""),
    # "...If so, what?"

    ("「………」",
    ""),
    # "..."

    ("もしかして、オレ、得意なことがない？",
    ""),
    # Maybe I'm really into something I'm good at?

    ("考えてみり痰ゃ、そうかも。",
    ""),
    # Think about it, phlegm, I see, sniff, and hot.

    ("げっ。",
    ""),
    # Oh no.

    ("じつはオレって、無能な人間なのか。",
    ""),
    # Actually, am I an incompetent person?

    ("「なににするの？」",
    ""),
    # "What are you going to do?"

    ("「堰ラ…」",
    ""),
    # "Dear..."

    ("「…は？」",
    ""),
    # "…teeth?"

    ("「堰ラー＝メン…」",
    ""),
    # "The weir ra-men..."

    ("「堰ラー＝メン？」",
    ""),
    # "Yellow Men?" 

    ("「堰ラー＝メン＜早食い勝縁負はどうだあっ！」",
    ""),
    # "Yahra-men <What about the winnings of fast eating?"

    ("「オッケー、いいわよ」",
    ""),
    # "Okay, it's nice."

    ("「えっ！？」",
    ""),
    # "What?!"

    ("あっさりＯＫする、綾∴香。",
    ""),
    # Aya∴Kasu is quick to go with it.

    ("な、なんだ、この余裕は。",
    ""),
    # What, what, is this leeway?

    ("もしかして、＝早食いが得意なのか？",
    ""),
    # Maybe = Is it good at eating quickly?

    ("「ま、まて。",
    ""),
    # "Well, well.

    ("やっぱり、『蛛大盛關り』堰ラー＝メン＜早食い勝縁負にしよう」",
    ""),
    # After all, "Great Bowl" Ira-men <Let's eat quickly and make a victory for the winner."

    ("「うん、わかった」",
    ""),
    # "Yeah, I understand."

    ("あらま、あっさりと。",
    ""),
    # Oh well, it's easy.

    ("「い、いや、さらに『メ辛』を追加しよう。",
    ""),
    # "I'll add some tampering and more "Super spicy."

    ("大盛關り『メ辛』堰ラー＝メン＜早食い勝縁負だ」",
    ""),
    # A large portion of "super spicy" yarn ramen <<A quick eater is a win-win situation."

    ("「いいわ」",
    ""),
    # "It's nice."

    ("ぐっ。",
    ""),
    # Wow.

    ("「じ痰ゃ、『蛛大盛關りモ辛艶ラー＝メン＜早食い勝縁負』でいいのね？」",
    ""),
    # "Is it okay to have a big savory savory roe with super spicy and shiny ramen <fast eating and winning the game"?"

    ("「お、おう」",
    ""),
    # "Oh, oh."

    ("「わかった、受けて立つわよ。じ痰ゃあ、氏試㍽合の冾日はいつにする？」",
    ""),
    # "Okay, I'll take it and get on. When will the test be held?"

    ("「…そ、そうだな」",
    ""),
    # "...Oh, that's right" 

    ("なんか、諱取り返しのつかない方阜に流齬れてしまったような気がする。",
    ""),
    # It feels like I've been in conflict with Fukukata Fu, which has no reciprocity.

    ("そもそもオレは『蛛大盛關りモ辛艶ラー＝メン』の≠早食いが得意なのか、おい。",
    ""),
    # Is it good at eating fast foods like "Great Large Large Moss Extremely Spicy and Spicy Ramen"?

    ("「じ痰ゃそれは、バーガー食べながらｌめよ。運動してるせいか、最近この桙時間に、どうもお腹が空くのよね」",
    ""),
    # "You're so excited to eat a burger, so I've been feeling hungry lately after all this time."

    ("「あ、そり痰ゃ太る前尅兆かもな。食岦わないほうがいいんじ痰ゃねーか？」",
    ""),
    # "Oh, there's a sign of sputum that you can skull a thick skull, or a skull. Isn't it better not to eat it, so skull?"

    ("「あら、平気諡よ。太迹らない体質だから翌＠」",
    ""),
    # "Oh, it's okay. I have a constitution that doesn't make you stronger, so the next day."

    ("「あっそ」",
    ""),
    # "Ahhh."

    ("「ふふっ」",
    ""),
    # "Heheh."

    ("微笑む綾∴香。",
    ""),
    # Aya∴Ka smiles.

    ("くそ。◯",
    ""),
    # Fuck.◯

]