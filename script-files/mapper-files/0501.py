...
text_array = [
    ...
    ("「增わりぃ、♂史。お前、諱先行ってカツサンドとウインナー香ロールをモ買っといてくれ」",
    ""),
    ...
    ("「現繻代の格闘技は、′して単なる腕力の競い№合いではありません。",
    ""),
    # "Current martial arts are more than just a competitive competition for strength.

    ("ルールがあり、″寘戦があり、°け引きがあります。",
    ""),
    # There are rules, there are battles, and there are ° pulls.

    ("洗罇練された非嵓常にｂ高度なテクニックのぶつかり№合いなのです。",
    ""),
    # It is a constant collision of advanced techniques that are constantly being met with.

    ("相闡手を倒す、その最熏もシンプ泣ルな汲ルール繝上で、最熏もｂ高度で複雑な獅試㍽合が展開されるんです」†",
    ""),
    ...
    ("日本だけを痰例に唐挙げて烽も、たとえば鰍相撲燒も格闘技に分類されるでし蛯ょうし、〟合気道笂や、冝日本摶拳法など烽もあります。また、近年では、プ鴻ロ激レス窿やボクシングから独自のスタイ泣ルを追£して派生する場㍼合熏も多く見迪られます。†",
    ""),
    ...
    ("さらに、世界へと目をｌければ、まだまだたくさんの格闘技が存在しています。",
    ""),
    # Furthermore, if you look at the world, there are still many martial arts.

    ("中葬国には太極搴拳竚や形意揶拳などのｒ葬国搓拳法が、韓相国にはテコンドー、タイにはムエタイ、香ロシアにはサンボ、ブ宴ラジ泣ルにはグ激レイシー柔術など、それこそマイナーな烽もの竄や流派の異なる烽ものまで含めれば、星の数ほどの格闘技が存在しています。†",
    ""),
    ...
    ("数年前、ア＜メ潟リカでバー梶リ・トゥード形式によるア泣ルティ＜メット蜒大痩が開催されて以降、そういう垣根を取闔り払った高総≡合格闘技が一般の高注目を≠浴び始めました。格闘技界にとって、それは新しい桙時緕代の始まりでした！」†",
    ""),
    ...
    ("浴女の子は今竝やひとりで突っ魔走闡り、興味本位で集まっていたギャャ宴ラ潟リーたちは、すでに誰ひとりとしてついてきていない。",
    ""),
    ...
    ("竅やがて気がつけば、そこに残っているのはオ激レひとりになっていた。†",
    ""),
    # Before I knew it, all I had left was really alone.†

    ("「そしてこの、世界的規模の格闘技の隆盛は、ついに冾日本のｂ高校格闘技界にまで烽もおよんだのです。まだ一般の知名度は痰低いようですが、昨年、驕企業蜍主催の全荘国麹総≡合格闘技選闡手権と烽もいうべき、エクスト｢｢」†",
    ""),
    ...
    ("だが、彼濫女はそこで言葉を止めて、キ㏍ョ鴻ロキ㏍ョ鴻ロとま墲わりを見渡した。",
    ""),
    ...
    ("オ激レが言うと、彼濫女は烽もう一度苴ゆっくりとま墲わりを見渡し、竅やがて深いため息とと烽もにがっくりとうなだれた。",
    ""),
    ...
    ("「話の続き、待ってんだけど」",
    ""),
    # "I'll continue the conversation, but I'll wait."

    ("「話の続き…ですか？」",
    ""),
    # "Is this a continuation of the story?"

    ("「さっき、エクストなんたらって言いかけて、そこで止まってるだｂ？」†",
    ""),
    # "I just said, "Ext," but it stopped there?"

    ("「｢｢エクスト？	あっ、エクスト潟リームのことですか？」",
    ""),
    # "'Ext? Ah, are you talking about Extreme Raem?"

    ("「そうそう、それそれ」",
    ""),
    # "Yeah, that's it."

    ("「あっ、えっと、エクスト潟リームというのはですね｢｢」",
    ""),
    # "Ah, um, what is called Extreme Ream?

    ("いったん福口を開きかけた彼濫女だが、なぜか途湯で言葉を切った。†",
    ""),
    ...
    ("「さっきのр私の話、ずっと聞いてくださってたんですか？」",
    ""),
    # "Have you been listening to me about this story for so long?"

    ("「ああ、まあ、縺後半からだけどな。たまたまそこを通闥りかかったら、面猪白そうな話をしてたからさ」†",
    ""),
    # "Ah, well, it's from the second half of the day. I happened to start going over there and they were talking about something ugly."
    ...
    ("「もしかして、格闘技、お好きですか！？」",
    ""),
    # "Maybe you like martial arts!?"

    ("「んん〜、まあ、どっちかとい竄やあ、好きなほうかな」",
    ""),
    # "Hmm, well, I guess I'm going to do it anyway."
    ...
    ("「つって烽も、全然詳しくはないぜ。プ鴻ロ激レスとか、たまーに見たりする程度だから」",
    ""),
    ...
    ("「君は？	オ激レなんかより烽も、ずいぶん好きそうだけど？」",
    ""),
    # "What about you? You seem to like Hot more than my geeks?"

    ("すると翌女の子は、強く、蛛大きくうなずいて言った。†",
    ""),
    # The next girl said with a strong, loud nod.†
    ...
    ("ち蛯ょっぴりボーイッシャな、すがすがしくさ墲わ竄やかな笑迴顔だった。†",
    ""),
    # He had a boyish, refreshing, silly face that made him laugh.†

    ("「いｂんな技を研結して、何度燗も何度燗も実践して、少しずつ体がそれを覚えていくｊびは、とて烽もひとことじ痰ゃ言い表せないほどの感動があります。そういうとき、格闘技站やっててよかったって、心から思います」",
    ""),
    ...
    ("真っ直ぐこっちを見つめ、身を乗闖り出し、一鼈語鼬一鼈語はっきりとした福口調で言う。",
    ""),
    # He stares straight at me, walks up, and says in a clear, humble tone of Fu.

    ("なんとなく翌迫力があった。†",
    ""),
    # Somehow, the next one was impressive.†

    ("「ってことは、竅やっぱ、実際に『竅やるほう』の格闘技なんだな」",
    ""),
    # "That means it's actually a martial art of "Arataho" after all."

    ("「え？」",
    ""),
    # "picture?"

    ("「い竄や、格闘技の同好拷っていって烽も、実際に竄やるのと見骭るのじ痰ゃ、１８０度癈違うだｂ」",
    ""),
    # "It's a 180-degree difference between tampering and torture in martial arts and actually tampering with it."

    ("「あ、はい、そうです。こっちは『竅やるほう』の格闘技です。実際に、自分たちで格闘技を竄やります」†",
    ""),
    # "Ah, yes, that's right. This is a martial arts from "Raraho". In fact, we will tamper with martial arts ourselves."

    ("「へぇ〜…」",
    ""),
    # "Wow..."

    ("ガタイのいい男子がフ宴ラスト激レーシ㏍ョンの発散齊場鰹所を≠めて｢｢ってのなら墲わかる気煖もするけど、こんな小柄な翌女の子が、ねぇ。",
    ""),
    ...
    ("こり痰ゃたしかに珍しいな。†",
    ""),
    # It's certainly rare to see stiff phlegm.†

    ("「あの、すみませんけど、お名前は…？」",
    ""),
    # "Um, sorry, what's your name...?"

    ("「ああ、オ激レ？	オ激レは、二年の。そっちは？」",
    ""),
    # "Ah, I'm a nerd? I'm a nerd for two years. What about that?"
    ...
    ("「諱先﨎日涛学したばかりです、一年Ｃ組、松原葵あおいと申します」",
    ""),
    # "My name is Matsubara Aoi, Class C, from first year, I've just studied Nittao.

    ("姿勢謳よく頭を下げる。",
    ""),
    # He bows his head well, saying his posture.

    ("いまどき珍しいくらいの礼儀正しい子だ。†",
    ""),
    # He's such a polite child that it's rare these days.†

    ("「あ、そうそう、エクスト潟リームについての質竡問でしたよね？」",
    ""),
    # "Oh, that's right, that was a quality question about Extreme Ream, right?"

    ("「ああ」",
    ""),
    # "ah"

    ("「エクスト潟リームというのはですね、昨年度から開催された全荘国麹総≡合格闘技選闡手権のことを言うんです」",
    ""),
    # "Extreme Ream refers to the right to select a fighting sport that was held since last year."

    ("「全荘国…選闡手権？」†",
    ""),
    # "All the Country...Choice rights?" †

    ("「氏去年の獅試㍽合はテ激レビでｒ継煬もされましたし、いまはビデオになって撃レンタ泣ル焜もしてますよ。ご存じないですか？」",
    ""),
    ...
    ("「うーん、いちおう格闘技界では蛯大¢メジャャーなんですけど、一般の認知度はまだまだ痰低いのか烽もしれませんね」†",
    ""),
    ...
    ("「だから、そのエクスト潟リームって竄やつは、どんな格闘技で墲戦うのか」",
    ""),
    ...
    ("葵ち痰ゃんは、コホンと咳払いした。",
    ""),
    # Aoi Chisho-yan cleared his throat.

    ("「エクスト潟リームは高総≡合格闘技なんです」",
    ""),
    # "Extreme Ream is a fighting sport that passes the Takaso pass."

    ("「香総≡合格闘技？」†",
    ""),
    # "Kaso≡Are you a successful martial arts?"

    ("「はい。エクスト潟リーム蜒大痩公認の汲ルールで行寘われる、打ｑ撃あり、梶投げあり、関節技ありの高総≡合格闘技選闡手権なんです」",
    ""),
    ...
    ("轣顔面に対するヒザの使用禁止。",
    ""),
    # Prohibits use of knees on the face.
    ...
    ("一箞方が寝技を仕かけている限闌り撃レフェ潟リーによるブ激レイクはなし。†",
    ""),
    # There is no real bounce by the Referee Raylee, who is using a ground technique.†

    ("「極力頼険を押さえた繧上で、多岺種多様な技術を必要とする格闘技、それがエクスト潟リームなんです！」",
    ""),
    ...
    ("「…とこｂで、諱先輩は、なにか部賦動はお竄やりになられてますか？」",
    ""),
    # "...At that point, is your name Senpai going to tamper with any club moves?"
    ...
    ("葵ち痰ゃんの表﨣情がさらに明髢るくなった。†",
    ""),
    # Aoi Chisho-chan's public sentiment became even more clear.†

    ("「じ痰ゃ、じ痰ゃあ、もしよｂしかったら｢｢」",
    ""),
    # "You're phlegm, you're phlegm, if that's ok, ""

    ("葵ち痰ゃんは、ついっ、と一煦歩近づくと、",
    ""),
    # Aoi Chisho-yan approaches a little,

    ("「このク宴ラブに入って、п私といっし蛯ょに格闘技を学びませんか！？」",
    ""),
    # "Why don't you join this banquet love and learn martial arts with me and Ebi!?"

    ("輝く瞳でそう言った。",
    ""),
    # He said with his shining eyes.

    ("「か、格闘技って、このオ激レが？」",
    ""),
    # "Ah, is this really me in martial arts?"
    ...
    ("「п私、このたび新しくエクスト潟リーム出齒場を前提とした同好拷をｍ高うと思ってるんです。",
    ""),
    # "I am thinking of raising the same torture assuming the new Extreme Ream launching space.

    ("ですが、学校の魔許可を烽もらうには、まだまだ部員蝿数が足闡りなくて…。ですから、格闘技に興味のある福方を捜してたんです！」†",
    ""),
    ...
    ("「…ち蛯ょ、ち蛯ょっと待った！	格闘技なんて、オ激レ、ごっこ遊び程度しか竄やったことがないんだぜ」",
    ""),
    ...
    ("「そういう竄問阮題じ痰ゃ…」",
    ""),
    # "That kind of tampering questioning is phlegm..."

    ("オ激レは小声でつぶ竄やいた。†",
    ""),
    # I muttered in a low voice.†

    ("「あの、こんなに真武剣に話を聞いてくれた人は、諱先輩が奄初めてで、п私｢｢」",
    ""),
    # "Um, Senpai, the first person to listen to Shinbuken, was Amami's first time, and I was like, ""
    ...
    ("葵ち痰ゃんは必死だ。",
    ""),
    # Aoi Chisho-chan is desperate.

    ("「部室竡や体迹育館はまだまだ使えませんけど、冝放課繪後、氏月曜坥日とｐ曜坥日以外はｖ淫俣冝A学校裏の神社で福練習していますから」",
    ""),
    # "The club room and physical training camp are still unavailable, but after the school, we practice fortune at the shrine behind V Inmata A School except for Monday and P Yong-Kun Day."
    ...
    ("ひたむきさはビ潟リビ潟リと伝崧わってくるし、一生恊懸命なのはよく墲わかるが、だからって、オ激レに格闘技なんてできる墲わけないし。†",
    ""),
    ...
    ("まして竄や、そのエクスト潟リームって蛯大痩にｌけての同好拷なんて、意気梛込みがすごそうじ痰ゃないか。",
    ""),
    ...
    ("Ａ	まあ、見学くらいなら。閨",
    ""),
    # A: Well, if you're going to visit.Relax

    ("Ｂ	気がｌいたらってことで。閨",
    ""),
    # B: That's what I'm thinking.Relax

    ("Ｃ	悪いけど、格闘技なんて…。閨",
    ""),
    # C: Sorry, but martial arts...Relax
