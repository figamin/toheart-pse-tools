...
text_array = [
    ("（マルチ	出初いイベント）閨（４月１２冾日	２桙時間目の休み桙時間）閨（渡闢り廊下）",
    ""),
    ...
    ("教室へ戻髢る途湯、カフェオレのスト鴻ローを口に魔当てながら階段を上っていると…、",
    ""),
    ...
    ("諱よよと、♀なかしげな足どりで、階段を上っている翌女生徒がいた。",
    ""),
    # There was a female student climbing the stairs, with a female figure in her legs, and a female student.

    ("重たそうな段ボール箱をふたつ、縺上下に重ねて運んでいる。",
    ""),
    # Two heavy cardboard boxes are carried over each other on top and bottom.

    ("足熨もとがよく見えないのか、つま先で一煦歩ずつ確認しながら、艨ゆっくり上っていた。",
    ""),
    # I wondered if my feet were visible properly, so I slowly climbed up, checking each step with my toes.
    ...
    ("まるで見計迪らったように、その翌女生徒は足をつまずかせ、体勢を崩した。",
    ""),
    # As if to be deliberate, the next female student tripped her leg and lost her position.

    ("「あっ、あわっ、あわわわわわっ…」",
    ""),
    # "Ah, ah, ah, ah, ah, ah, ah, ah, ah, ah, ah, ah, ah, ah, ah, ah, ah, ah, ah, ah, ah, ah, ah, ah..."

    ("ふらふらと体を動かし、必死にバランスをとうとする翌女生徒だったが、最纃後はやっぱりこらえきれず、縺後撃へ落下した。",
    ""),
    ...
    ("諱取りあえずはセーフ…と。",
    ""),
    # It's safe to ask for your name...

    ("なんとなくこうなる予感がして、あらかじめ高注意を払っていたのが幸いした。",
    ""),
    # I had a feeling that this was going to happen, and I was fortunate to have been given high caution in advance.

    ("そうでもなき痰ゃ、そう都∮合謐よく、",
    ""),
    # And so, there's no heat, so it's a good idea to combine it with other people,
    ...
    ("受け止められるわけが、",
    ""),
    # The reason why it is accepted is
    ...
    ("「いつまで落ちてんだよ」",
    ""),
    # "How long will the lion be falling?"
    ...
    ("「わっ、わわ、わたしったらホントにドジで、いつもいつも失敗ばっかりしち痰ゃって、そのうえ人様にまでご迷惑を｢｢」",
    ""),
    ...
    ("「どうだ？	氏落ちｒいたか？」",
    ""),
    # "What do you think? Did you miss Mr?"
    ...
    ("見骭ると、その子の耳には、秩白い大きな金属製のｏ闖りのようなものがついていた。",
    ""),
    # When he looked over, the child's ears were covered in a large, ordered, hot metallic hot spring.

    ("蛛大きなヘッドホンという感じで、頭の後撃まで突き出ている。",
    ""),
    # It looks like a big headphone, and even sticks out after the head is shining.
    ...
    ""),
    # "Ah, is this? It's a sensor."

    ("女生徒は、にっこり微笑んで嘯答えた。",
    ""),
    # The female bather replied with a smile.

    ("「…セ、センサ〜？」",
    ""),
    # "...Se, Sensor?" 

    ("「はい。でも本末当は、人間の方と見間違われないために付けられているそうです」",
    ""),
    ...
    ("耳にセンサーがあり、しかも人間じ痰ゃないなんていったら…。",
    ""),
    # If you say there is a sensor in your ears and that even hot water is human...

    ("「もしかして、いま、うちの学校で試験健の新型′メイド鴻ロボってのは…」",
    ""),
    # "Maybe it's the new Maid Korobo at my school right now that Shio Examination Ken is...
    ...
    ("女生徒は、にっこり笑ってそう言った。",
    ""),
    # The female bather said with a smile.

    ("「すみません、自己紹譓介が遅くなってしまいました。",
    ""),
    # "Sorry, I've been late for my self-introduction.

    ("わたし、このたび、みなさんといっしょにお勉強させていただくことになりました、汎用アンド鴻ロイドの『ＨＭＸ−１２型』といいます。残り１週間という短い冾期間ですが、どうかよしくお願いします」",
    ""),
    ...
    ("「さんですかー。素敵なお名前ですねー」",
    ""),
    # "Yes, you know? That's a wonderful name."
    ...
    ("まあ、梶相闡手に名前をたずねたら、必ずそう返すようにプ鴻ログラムされてるのかもな。",
    ""),
    ...
    ("「そう言ってもらえると嬉しいです。現在の高ロボット技術のｐ芋垂Wめ、可能な限り人間に近い≠メイド鴻ロボをｍ高うというのが、わたしの開発コンセプト辜らしいですから」",
    ""),
    ...
    ("「ふ〜ん、なるほどね。うん、たしかに人間そっくりだ」",
    ""),
    # "Hmm, I see. Yes, he certainly looks just like a human."
    ...
    ("いままでの≠メイド鴻ロボに感じられた、どこか人形っぽい部分が見受けられない。",
    ""),
    # I can't see the doll-like part that feels like the previous ≠Maid Korobo.

    ("あの耳のｏ闖り（センサーだっけ）がなき痰ゃ、本末当に人間と区別がつかないぜ。",
    ""),
    # Without those ears (or sensors), it's hard to tell the difference between them from humans.

    ("もしかして、＝メイド鴻ロボと偽って、じつは≠普通の翌女の子がｏ闖りを付けてるだけなんじ痰ゃないか。",
    ""),
    # Perhaps it's because they pretend that Maid Korobo is a maid, but in fact, the girl who normally says that she's just plain slutting is just slutting.

    ("なんて、疑ってしまいたくなるほどだ。",
    ""),
    # That's so much that it makes me want to doubt it.

    ("なるほど、最新型か。",
    ""),
    # I see, the latest model.

    ("「でも、階段から転げ落ちたりするなんて、結構ドジな最新型だな、お前は」",
    ""),
    ...
    ("わたし、最新型のくせに、なぜか失敗が多くて…。せっかくいいと高性能にｍってもらっているのに、これじ痰ゃ、開発者の方々に申し訳ないです」",
    ""),
    ...
    ("うーむ、前きに生きる高ロボットだな。",
    ""),
    # Hmm, it's a high-level robot that lives in front of the board.
    ...
    ("「そういえばわたし、お仕鮪事の途湯でした。その箱をコピー室まで持っていかないと」",
    ""),
    # "By the way, I was a tuna broth. I have to take the box to the copy room."

    ("そう言うと、マルチは慌てて床に転がった段ボール箱の前に行き、うんしょっ、とそれを持ち上げた。",
    ""),
    ...
    ("どさっと、もう１個の上に重ねると、二段になったそれをいっぺんに持とうとする。",
    ""),
    # I quickly stack it on another piece of cake and try to hold it in two layers at once.
    ...
    ("「ま、香ロボットっていっても、やっぱり得闢手不得闢手があるんだ。マルチは翌女の子だから、力仕鮪事にはｌいてないのさ。そのぶん得意なことでみんなのお役に立ち痰ゃーいいんだよ。得意なことは何なんだ？」",
    ""),
    ...
    ("「得意というわけじ痰ゃないですけど、お掃恆除は大好きです！」",
    ""),
    # "I don't have to deal with the idea of ​​what I'm good at, but I love the ejaculation and removal of the seduction!"

    ("「そうか。だったら、そういうので頑張ったらいいんじ痰ゃないのか？」",
    ""),
    # "I see. If that's the case, then why don't you try hard, do you have to phlegm?"
