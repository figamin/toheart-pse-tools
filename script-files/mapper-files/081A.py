...
# Source file: 081A.txt

text_array = [
    ("（レミィ	冝日付イベント１	ゲーセン解煢説あり）",
    ""),
    ...
    ("このゲーセンは、この辺阨りでは一番蜚大きく、潤ワンフ鴻ロアの店燗内には、かなりの数のゲーム機が設置されている。",
    ""),
    ...
    ("「そうネ。じ痰ゃあ、どれで遊べばイイ？」",
    ""),
    # "Yeah. I've got phlegm, which one should I play with?"
    ...
    ("オレも結構遊んでるゲームなので、なにかとアドバイスはできるはずだ。",
    ""),
    ...
    ("「…か、氈勝ち？	もしかして撃レミィ、おめー、ゲーセンで遊んだことないのか？」",
    ""),
    # "...Oh, do you win? Maybe Ekire Remie, have you never played in the arcade?"
    ...
    ("今の桙時緕代、ゲームの遊び方を知迺らないってのも、ある意味、天然記念物だよな。",
    ""),
    ...
    ("オレは、レバーやボタンの扱い方、果てはコイン投涛からゲームスタートまでを解煢説した。",
    ""),
    ...
    ("しかしズブの素人である撃レミィにどこまで理解させられたかは、ちょっと疑站問だ。",
    ""),
    ...
    ""),
    # Karemi silently looked at the DeC Mosaic screen.

    ("「ヘイ、レミィ！	インサートコイン、アンド、プッシャスタート！」",
    ""),
    # "Hey, huh remi! Insert coin, and, pusher start!"
    ...
    ("「おいおい、構えるのはゲームが始まってからでいいんだよ」",
    ""),
    # "Hey hey, you can only stand there once the game starts."

    ("「そうなの？」",
    ""),
    # "Is that so?"

    ("「あったりまえだ。まだカネも入齠れてねーうちから構えるрヤツがあるか！」",
    ""),
    ...
    ("レミィは少し緊張した面摶持ちで百円玉を取り出すと、梶投涛茁口へ冾放阨り桙込んだ。",
    ""),
    ...
    ("「右濶手のボタンだよ」",
    ""),
    # "It's the button on the right hand."

    ("ボタンを指差すと、レミィは、ボタンをペシッと叩いた。",
    ""),
    # When he pointed at the button, Remy slammed the button.
    ...
    ("でっかいおっさんの叫びで、敵は全滅。",
    ""),
    # The big old man's cry destroys the enemy.
    ...
    ("状況がつかめず、レミィはあたふた。",
    ""),
    # Unable to grasp the situation, Remy was in a flutter.

    ("「＝普通の攻ｍ撃はこっちだ」",
    ""),
    # "=This is the normal attack and M attack."
    ...
    ("そんなことはお構いなしで、レミィはペシペシとボタンを叩く。",
    ""),
    # Without caring about that, Karemi hits the button.

    ("「おいおい、始まってから少しも動いてないぞ。",
    ""),
    # "Hey hey, the hot water hasn't moved since it started.

    ("レバー操総がお留逞守だぜ」",
    ""),
    # Lever Manners is the one who is staying at the lord."

    ("「忘齧れてました！」",
    ""),
    # "I was forgetting!"

    ("香注意してやると、レミィの操る魔法使いは、右へ左へでたらめな方阜に動き始めた。",
    ""),
    ...
    ("さすがにザコの数が増えてきた。",
    ""),
    # The number of weaklings is increasing.

    ("今度は、２、３匹まとめて嘯爆風にｕまれる。",
    ""),
    # This time, two or three fish are all sniffed by the overflowing wind.
    ...
    ("…パ純ワーアップアイテムが出た。",
    ""),
    # ...Power-purpose work-up items have been released.

    ("アイテムは斜めに画面亦を飛び回驩る。",
    ""),
    # Items also fly around the screen diagonally.

    ("レミィは、それを器用に避けている。",
    ""),
    # The wa Remie avoids it for a sensible purpose.

    ("「それは避けるんじ痰ゃなくて、諱取骼るんだよ」",
    ""),
    # "I'm not going to avoid that, I'll just get my name off."
    ...
    ("「諱取齊れるの？」",
    ""),
    # "Can you get a name for yourself?"

    ("真逅顔で聞いてくる。",
    ""),
    # He asks with a Mako face.

    ("「ああ。そいつを取齊れば、パ純ワーアップするんだよ」",
    ""),
    # "Yeah. If you take that guy, you'll get a boost."

    ("オレが言うと、レミィはじっと自分の手元と画面を見芟比べ始めた。",
    ""),
    ...
    ("「どうした？	諱取骼るんだよ」",
    ""),
    # "What's wrong? I'm going to get my name on you."

    ("「…諱取齊れません」",
    ""),
    # "...I won't be able to collect my name."
    ...
    ("「どのボタンで取骼るの？」",
    ""),
    # "Which button do you use to get it?"
    ...
    ("アイテムを回精収できた魔法使いは『パ純ワーアップ』のボイスとともに攻ｍ撃が変化した。",
    ""),
    ...
    ("ゲームのボイス諠より、さまになった発音で、レミィが声を上げる。",
    ""),
    # The voice of the game is based on the voice of the game, and the sound of the sound is raised.

    ("「、変﨑わった、変﨑わったｃ！」",
    ""),
    # "It's turned on, it turned on c!"
    ...
    ("レミィはいつもの太陽のような笑みといっしょに、ほっぺにチャーしてくれた。",
    ""),
    ...
    ("諱取りあえず、レミィにｊんでもらえたし、結果オー堰ライにしておこう。◯",
    ""),
    # Without getting accused of the name, I was jotted by the Remy and got a hot pot, so I'll leave it at Oh-Yay.◯
