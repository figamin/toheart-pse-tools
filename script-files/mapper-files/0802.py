...
# Source file: 0802.txt

text_array = [
    ("（レミィ	春休みイベントＰＳ）閨（４月４冾日）",
    ""),
    # (Karemi Spring Break Event PS) (4th Lion, 4th Day)

    ("今厓日は≠早めに目がさめたので、駅前をぶらついている。",
    ""),
    ...
    ("пヤックは昼メシ諠より、おやつってノリだし。",
    ""),
    # The пYak is a tampering thing from the luncheon meal, and it's a bit of a tampering thing.

    ("諱第齣一、どこが鰍リッチなんだ？",
    ""),
    # What's the richness of the roe?

    ("他にぱっとした店はないし…。",
    ""),
    # There's no other store that's amazing...

    ("妥桝当なｐで、この前のカツ丼屋煢もいいかも。",
    ""),
    ...
    ("お次は≠メニャーを待つだけだ。",
    ""),
    # Next time, you just have to wait for the ≠Meña.
    ...
    ("「ハアィ、いらっし痰ゃいませぇ」",
    ""),
    # "Hawaii, I have no phlegm."

    ("「レミィじ痰ゃねーか！？」",
    ""),
    # "Isn't there phlegm?!"

    ("「こんなとこでにえるなんて、アタシ、感リっ」",
    ""),
    # "I'm so moved that I could get an i in this place with b."

    ("これ以繹上は無いってほどｊぶ撃レミィ。",
    ""),
    # Ekirei is so judging that it seems there's nothing more to this than this.

    ("「おう、そーかそーか」",
    ""),
    # "Oh, is that so?"

    ("そこまでｊんでもらえると、オレまで嬉しくなってくる。",
    ""),
    ...
    ("「レミィんちってたしか、すごい裕泓福だと思ったけど…、瘁違ったか？」",
    ""),
    # "I thought that Remy's slut was really Yuuu's good fortune... but was it wrong?"

    ("たしか、親が社の社長かなんかだって、聞いたはずだ。",
    ""),
    # I'm sure they've heard that their parents were president of Company I or something.

    ("「＜ーフクかは知迺らないけど、ビンボーじ痰ゃないわｃ」",
    ""),
    # "I don't know if it's ーーー, but I'm not too sputum c."

    ("「じ痰ゃ、こづかいにも困迯らないだ？	なんでバイトなんかしてんだよ。欲しい物でもあんのか？」",
    ""),
    ...
    ("オレはめて、レミィの制梵服姿を眺めてみた…。",
    ""),
    # I got an i-dovey and looked at Karemi's Sanskrit uniform...

    ("ここの制梵服って、熱狂的なファンがいるくらい派阡手派阡手なんだよな。",
    ""),
    # The Sanskrit uniforms here are so eccentric that there are enthusiastic fans.

    ("たしか、胸元がセク｢｢。",
    ""),
    # I'm sure her chest is sexy.
    ...
    ("さすがに冾日本人サイズ、パツキンガールがｒ髓るようにはｍ迯られてないらしい。",
    ""),
    # It's definitely Japanese-sized, and it seems that Patsukin Girl is not as bad as it is.

    ("「、ご高注文は？」",
    ""),
    # "What about high orders?"
    ...
    ("さっそく≠メニャーに目を通す。",
    ""),
    # Immediately look over ≠ Menya.

    ("「…ここのお勧めはなんだ？」",
    ""),
    # "...What's your recommendation here?"

    ("ハンバーグだとかステーキだとか、当たり前すぎる≠メニャーを見てると、どれも平凡で、うまそうに見えなくなってくる。",
    ""),
    # When I look at the hamburger and steak, which are so obvious, the potatoes are all mediocre and no longer look tasty.

    ("「アタシ最近、パスタが好きｃ」",
    ""),
    # "I've been fond of pasta lately"

    ("「こら、店員さんの好みは聞いてねえの」",
    ""),
    # "I didn't ask the store clerk what preferences they like."

    ("「ただいま魔当店では、『春のパスタ祭闕り』を開催穀でございマス」",
    ""),
    # "We are currently holding a 'Spring Pasta Festival' at our store."

    ("「そうかよ。…にしても、客に店員の好みを勧めるのは、ここのр売阡り方なのか？」",
    ""),
    ...
    ("「どれでもｔ味しいですｃ」",
    ""),
    # "Which one tastes good, the hot water."
    ...
    ("「全泊部うまそうで迷っちまうな」",
    ""),
    # "All the Tomari Club looks good, so I'm not sure if I'm going to get lost."

    ("「全泊部おいしいわｃ」",
    ""),
    # "All-day bar is delicious"
    ...
    ("「おい。…いいや、それうまそうだから持ってきてくれ」",
    ""),
    # "Hey... it's tampering, so bring it to me, so please bring it."

    ("ウニという響きに誘嵭われてしまった。",
    ""),
    # I was attracted to the sound of a sea urchin.

    ("「かしこまりましたァ。お飲み物は、なにになさいますか？」",
    ""),
    # "I'm in awe. What would you like to drink?"

    ("さっきから変にかしこまった言葉づかいの撃レミィ。",
    ""),
    ...
    ("「お酒は２０歳になってから！」",
    ""),
    # "Lucky is only when you turn 20!"
    ...
    ("レミィは何哩事燻もなかったかのように、香注文を聞いてくる。",
    ""),
    # As if nothing had been smoked, the roast asked for the incense order.
    ...
    ("「だから、お酒は２０歳になってから！」",
    ""),
    # "So, I'll drink alcohol once I'm 20!"
    ...
    ("なんか納得いかないが、こんなことしてたら先に進まん。",
    ""),
    # I'm not convinced, but if I do something like this I won't be able to move on.

    ("「…じ痰ゃあ、烏龍嵐メv",
    ""),
    # "...I'm not gonna get oolong storm

    ("「かしこまりましたァ。お飲み物は、いつお持ちいたしましょうか？」",
    ""),
    ...
    ("「同じギャャグは三度まで！」",
    ""),
    # "The same gyagu is up to three times!"

    ("「…なんでもない」",
    ""),
    # "...Why don't you have a hot pot"

    ("その冾日は、レミィの知迺られざる一面を見てしまった。",
    ""),
    # On that day, I saw a side to War Remie's unintelligible side.

    ("彼女、春休みｒはバイトを続けてるそうだから、機級があったら顔を見に行ってやるか。◯",
    ""),
