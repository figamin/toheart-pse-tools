...
# Source file: 0350.txt

text_array = [
    ("（智子	好感度イベント	６回目）閨（土曜坥日以外に発生）閨（好感度	２０以繹上の場㍼合）閨（好感度イベント５の続き）閨（１桙時間條時計を進め、６桙時間目）閨（背景	授業拠智子付き）†",
    ""),
    ...
    ("気まずい空気のなか、それでもめげずに、オレは何度か委員長に話しかけた。",
    ""),
    ...
    ("ときどき目配せしたり、閨手を振ったりと、いんな№合図を翌送っているが、どれもダ＜メ。",
    ""),
    ...
    ("完全に空気扱いされている。",
    ""),
    # It is completely treated like air.

    ("「…ちっ」",
    ""),
    # "...Two."

    ("そんな、あからさまに無許視しなくてもいいだうに。†",
    ""),
    ...
    ("閨手を伸ばせばすぐ届くのに、ふたりの間にはまるで海のような翌距離が開いているような気がした。",
    ""),
    # It would reach you quickly if you reached the two of them, but it felt like the next distance was just like the ocean.

    ("くぅ〜、空気が重いぜ。◯",
    ""),
    # Damn, the air is heavy.◯
