# Generated array of Japanese text with English comments
# Source file: 03A5.txt

text_array = [
    ("（智子	回秤数イベント	帰闍り道	ＬＶ１）閨（４獅月）閨（本屋煢内）†",
    ""),
    # (Tomoko's Scales Event Return Road LV1) (4 Shizuki) (Inside the bookstore)†

    ("帰闍りにぶらっと本屋へ立ち寄闃り、雑庶誌を立ち読みしていたときのこと。",
    ""),
    # This was when I was strolling around at a bookstore and browsing miscellaneous magazines.

    ("読んでいた雑庶誌から、ふと店の外に汲視諮を移したとき、",
    ""),
    # When I suddenly moved from the sash magazine I was reading outside the store,

    ("「ん？	あれは…」◯",
    ""),
    # "Hmm? That's..." ◯

    ("偶然、前の道を委員長が通闥りかかった。",
    ""),
    # By chance, the chairman interjected the previous path.

    ("声で烽もかけようかと思い、雑庶誌を棚に戻しかけたとき、委員長がひとりじ痰ゃないことに気がついた。",
    ""),
    # I thought I'd also put some hot water in my voice, and as I nearly returned the miscellaneous magazine to the shelf, I realized that the chairman alone had no phlegm.

    ("委員長の横には他校の制梵服をｒた男がいて、ふたりいっし蛯ょに並んで烽歩いていた。†",
    ""),
    # Next to the chairman was a man wearing Sanskrit uniforms from another school, and the two of them were walking alongside each other's thud.†

    ("誰だ？",
    ""),
    # Who is it?

    ("もしかして、彼＃氏とか？",
    ""),
    # Maybe he's Mr. #?

    ("男の福方はしきりになにかを話しかけている様子だが、委員長の福方は痰例によっていつ烽ものポーカーフェイスでそれを聞き流しているといった状況だ。",
    ""),
    # The man Fukukata seems to be constantly talking to him about something, but the chairman Fukukata is always listening to it with a hot poker face.

    ("なんだか強引に言い寄迥られているように烽も見える。†",
    ""),
    # It looks like Ho is being forced to say something and is being approached.†

    ("男の顔を烽もっとよく見謖ようと思ったとき、ふたりは店の前を通闥り過ぎ、見えなくなった。†",
    ""),
    # Just as they wanted to see the man's face more closely, they walked too far in front of the store and were no longer visible.†

    ("うーん。",
    ""),
    # Hmm.

    ("委員長の意外なシチャエーシ㏍ョン。",
    ""),
    # The chairman's surprising tranquilizers.

    ("ち蛯ょっと気になるな。",
    ""),
    # I'm really curious.

    ("Ａ	縺後を追ってみる。閨",
    ""),
    # A: Let's follow after the scene.Relax

    ("Ｂ	プ宴ライベートには立ち入逑らない。閨",
    ""),
    # B Do not enter and visit the private banquet live.Relax

]