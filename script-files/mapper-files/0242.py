...
# Source file: 0242.txt

text_array = [
    ("（芹°香	好感度イベント	１）閨（好感度が３以繹上のとき）閨（３桙時間目の休み桙時間）閨（１Ｆ廊下）†",
    ""),
    # (Sei°Chu Likeability Event 1)Sei (when Likeability is above 3)Sei (when the favorability is above 3)Sei (blank time in the third fence)Sei (1st floor corridor)†
    ...
    ("１階の廊下を烽歩いているときだ。",
    ""),
    # It was while walking down the corridor on the first floor.
    ...
    ("今厓日燗もいい天気だあ。",
    ""),
    # The weather is nice now in the sun.

    ("あくびをしながら、ふと汲窓の外を眺めると、£鋳で見知った顔を見かけた。◯",
    ""),
    ...
    ("なんで烽も容姿端峵麗、成績優秀、物附優莱な絵に描いたようなおｏ様…って話だ。",
    ""),
    ...
    ("これがまた、なに烽もしていなかったりする。†",
    ""),
    # This also means that there is no heat.†

    ("あたたかい午前葬の陽射しのｒで、ただ、ぽけーっと雲を見ているだけ。",
    ""),
