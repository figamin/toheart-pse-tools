# Generated array of Japanese text with English comments
# Source file: 02A6.txt

text_array = [
    ("（回秤数イベント	１Ｆ	ＬＶ１）閨（選択綜ＡＢＣの続き）",
    ""),
    # (Scale Event 1F LV1) (Continued from Selection ABC)

    ("オレと先輩は、そのままなんでもない桙時間を過ごした。",
    ""),
    # My senior and I just spent some hot and bouncing time.

    ("それから５分ほどして、時計を見た先輩が、",
    ""),
    # About five minutes later, when my senior looked at the clock,

    ("そそ桙時間です、と立ち上がった。",
    ""),
    # He stood up saying, "It's time for b, it's time for b, it's time for b, and I got up.

    ("「………」",
    "…"),
    # "..."

    ("「あっ、迎えがｗ磯翌ﾌか？	じ痰ゃあ、オレも校門までいっしょに行くよ」",
    ""),
    # "Ah, I'll be able to welcome you lol? Is it the next day? I'm going to go to the school gates too." 

    ("ふたりで校門前までやってｗたとき、",
    ""),
    # When the two of us tampered with each other in front of the school gates,

    ("「おｏ様、お迎えにあがりました」",
    ""),
    # "Oh, I've come to pick you up."

    ("停車していた鰍リムジンの陰から、ぬっと、お迎えのじーさんが姿を現した。",
    ""),
    # The old man who was greeted by the carpenter appeared from behind the locker limousine, which had been stopped.

    ("「………」",
    "…"),
    # "..."

    ("「え？	失邇礼しますって？	うん。じ痰ゃあ、また」",
    ""),
    # "Huh? I'm sorry? Yes. I'm sorry, I'll give you some phlegm." 

    ("諱先輩はぺこっと礼をして、車に乗った。",
    ""),
    # Senpai gave a deep bow and got into the car.

    ("じーさんも運転席に乗闖り桙込んで、普黒塗闢りの鰍リムジンは静かに魔走闡り去っていく。",
    ""),
    # Old man also got into the driver's seat and got into the driver's seat, and the limousine, which was painted in plain black, quietly leaned away.

    ("お城に住むお姫様と一般飼庶民。",
    ""),
    # A princess and ordinary people living in the castle.

    ("そんな感じ。",
    ""),
    # That's about it.

    ("あらためて住む世界の痰違いを感じるよな。◯",
    ""),
    # I can feel the difference in phlegm from the world I live in once again.◯

]