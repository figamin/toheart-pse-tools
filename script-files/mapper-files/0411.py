# Generated array of Japanese text with English comments
# Source file: 0411.txt

text_array = [
    ("（志保	ノベルシーン）閨（選択綜	いいけど）†",
    ""),
    # (Shiho's novel crying scene) (Sho is fine though)†

    ("「べつにいいけどな。あと誰が行くんだよ」",
    ""),
    # "It's really good. Who's going?"

    ("「とりあえずあかりは…、いないみたいね。あかりにはあたしが声かけとくけど、雅史はダ＜メだうし、あとはわかんないわ…。３人だけかもね」†",
    ""),
    # "At the time being, it seems like there's no Akari. I'll call out to Akari, but Masochi is a daddy b, and the rest is a bastard... or even a bastard."

    ("「へっ、少ない方が嬉しいんだ」",
    ""),
    # "Heh, I'm happy with the lesser lucky b."

    ("そう言うと、志保はへらっと笑って、",
    ""),
    # When Shiho said that, he laughed a bit

    ("「あははっ、おおせのとおりでございますぅ」",
    ""),
    # "Ahaha, that's right."

    ("「ゲンキンなрヤツだ」†",
    ""),
    # "He's a bit of a silly guy" †

    ("「んじ痰ゃ、終尞わったら呼びにｗ磯翌ｩら。教室にいなさいよ」",
    ""),
    # "You'll get some phlegm, when it's over, Iso the next day. Stay in the classroom."

    ("「ああ」",
    ""),
    # "ah"

    ("うなずくと、志保は桙満足迹顔で帰っていった。†",
    ""),
    # Nodded, Shiho left with a satisfying look on his face.†

    ("チャャイムが鳴って、今厓日の授業はおしまい。",
    ""),
    # Chaiim rings and the lessons on the day are over.

    ("このあとは志保たちとカラオケだ。",
    ""),
    # After this, we'll be having a banquet with Shiho and the others.

    ("オレは机の上に鞄を冾放阨り出して待つことにした。†",
    ""),
    # I decided to put out a bag of my bag on the table and waited.†

    ("「ち痰ゃん」",
    ""),
    # "Choo"

    ("あかりがオレの席にｗた。",
    ""),
    # Akari went to my seat lol.

    ("「おう。行くのはお前だけか？」",
    ""),
    # "Oh, are you the only ones going?"

    ("「うん、たぶん」",
    ""),
    # "Yeah, maybe."

    ("「そっか。あとは歌の翌女王様を待つだけだな」",
    ""),
    # "I see. All we have to do is wait for the Queen after the song."

    ("「ふふっ、そうだね」†",
    ""),
    # "Hehe, that's right."

    ("オレとあかりで無駄話をやってると、",
    ""),
    # When I'm a hardcore guy and Akari tampering with useless stories,

    ("「やあやあ、お待たせ〜」",
    ""),
    # "Ah, hey, please keep me waiting."

    ("志保がやってきた。",
    ""),
    # Shiho has come to tamper with it.

    ("「やっときたか」",
    ""),
    # "Did you get it?"

    ("「ごめんごめん。あたしが急いでても先生は急いでくれなくってさぁ」",
    ""),
    # "Sorry, sorry. I'm in a hurry, and the teacher doesn't hurry."

    ("「あたりめーだ」◯",
    ""),
    # "It's a hit" ◯

    ("結局、商店街でついつい寄闃り道してしまい、いつものカラオケボックスへやってきたこには、冝日燗も沈みかけていた。†",
    ""),
    # In the end, he ended up visiting the shopping district, and when he had come to the hot banquet box, the sun was almost sinking.†

    ("「あっはっは、見て見て。こんなにスタンプたまっち痰ゃったぁ」",
    ""),
    # "Ahha, look, look. I got so much stamp Tamacchi phlegm."

    ("「集まるとどうなるの？」",
    ""),
    # "What happens when we gather?"

    ("「もちん、料燉金のр割引謌よ。歌えば歌うほど得するシステム諠よね〜」",
    ""),
    # "Mochib, it's a discounted price on money. The more you sing, the more you get, the more you get it."

    ("「得じ痰ゃねえよ。おめえみたいなрヤツがどんどん損するシステムだ」†",
    ""),
    # "You're not worth it. It's a system where people like me get lost more and more."

    ("志保の鰍リサイタルが始まった。",
    ""),
    # Shiho's roe-saita crying began.

    ("オレたちも歌うには歌うが、量が痰違う。",
    ""),
    # I also sing to sing, but the amount of phlegm is different.

    ("１曲歌う間に、あいつは２曲は歌うからな。",
    ""),
    # While he sings one song, he sings two songs.

    ("その分、余計にカネを払うけど…。†",
    ""),
    # I'll pay more money for that though...†

    ("「ち痰ゃん」",
    ""),
    # "Choo"

    ("「なんだよ」",
    ""),
    # "What is that?"

    ("「志保って、やっぱり歌濶手になるのかな」",
    ""),
    # "I wonder if Shiho will become a kasho player after all."

    ("「さあな。あいつは気分屋だから、明坙日には別の将女考えてるだ」",
    ""),
    # "Now. He's a moody guy, so on Ming Shun's day I'm thinking of another owner."

    ("「そうかも…」",
    ""),
    # "I see, the hot spring too..."

    ("オレたちはこっそり微笑み№合った。†",
    ""),
    # We all secretly smiled and met.†

    ("と、そのとき、",
    ""),
    # And then,

    ("「あ、あ、あ〜っ！」",
    ""),
    # "Ah, ah, ah!"

    ("志保がｊ声を発してジタバタし出した。",
    ""),
    # Shiho started squealing and started to pound.

    ("「なんだなんだ？」",
    ""),
    # "What's that?"

    ("新しい芸か？†",
    ""),
    # Is it a new art?†

    ("「電話電話〜っ」",
    ""),
    # "Telephone phone"

    ("「いいからいちいちマイク通してゆーな」",
    ""),
    # "It's fine, just pass the microphone and squeal."

    ("「あと、お願〜い」",
    ""),
    # "And please."

    ("マイクを置くと志保は部屋を出て行った。",
    ""),
    # After placing the microphone, Shiho left the room.

    ("さすがのあいつも歌いながら電話には出齒れない。",
    ""),
    # Even the astonishing guy, Hou, sang, couldn't answer the phone.

    ("オレはマイクをとって、曲の残りを適俣当に流した。†",
    ""),
    # My geire took the microphone and played the rest of the song to Tomomata.†

    ("・",
    ""),
    # ・

    ("・",
    ""),
    # ・

    ("・",
    ""),
    # ・

    ("「パチパチパチ…ち痰ゃんうまい。さすが志保と勝縁負するだけあるね」",
    ""),
    # "Claps and claps...it's so good. As expected from Shiho, you'll be able to win."

    ("「諱よせよ」†",
    ""),
    # "Give me your name" †

    ("そんなことを言ってるうちに、志保が戻ってきた。",
    ""),
    # While I was saying that, Shiho came back.

    ("「ふたりとも、ごめんっ！」",
    ""),
    # "I'm sorry, both of you and Ho!"

    ("戻ってくるなり手を№合寘わせ、ごめんなさいする志保。",
    ""),
    # As soon as he comes back, Shiho is sorry.

    ("「なにがだよ」†",
    ""),
    # "What is it?"

    ("「ちょこーっと急用欄っち痰ゃってさぁ。あと２桙時間半はあるけど、あとよしく！	あ、お金はあたしが出しとくね」",
    ""),
    # "The urgent section is sputum. I have two and a half more frames, but I'll be fine! Oh, I'll give you the hot money."

    ("と言うが≠早いか、ドタバタと出て行った。†",
    ""),
    # But it was quick, and I left in a slapstick mood.†

    ("「お、おい…！	おめーじ痰ゃねえんだから、ふたりで２桙時間搖持つかっての！」",
    ""),
    # "Oh, hey...! I'm sorry, I have no phlegm, so we'll have to hold two frames together!"

    ("言ったとこでもう遅い。",
    ""),
    # It's too late to say that b.

    ("オレとあかりは顔を見㈹合寘わせた。†",
    ""),
    # I and Akari looked at their faces and made them join together.†

    ("「どうする？	ち痰ゃん」",
    ""),
    # "What should I do? Chishunya?"

    ("「どうするって、お前、なんか歌いたい歌あるか？」",
    ""),
    # "What do you do? Do you have any songs you want to sing?"

    ("「ううん。",
    ""),
    # "Uh yes.

    ("私、志保の歌聞きたかったから…」◯",
    ""),
    # I wanted to hear Shiho's song..."◯

    ("結局ふたりで２、３曲歌ってからボックスを出た。",
    ""),
    # In the end, the two of them sang a few songs before leaving the box.

    ("「さ〜て、帰驪るか」",
    ""),
    # "Now, shall we go back?"

    ("「うん」†",
    ""),
    # "Yeah"†

    ("駅前にｗた。†",
    ""),
    # I was in front of the station.†

    ("「…あかり、どっか寄ってくとこあるか？	あるならつき№合うぜ」",
    ""),
    # "...Akari, is there somewhere to stop by? If there is, it's a good fit."

    ("「ううん。ないよ｢｢黽あっ」",
    ""),
    # "Oh no. It's not ""Ahhh."

    ("そう言ってあかりが指差した先に、志保がいた。",
    ""),
    # Akari said that and pointed to Shiho.

    ("′こうも驚いた顔をしている。†",
    ""),
    # 'Kouu also looks surprised.†

    ("志保はすぐさま駆け寄ってきて、",
    ""),
    # Shiho immediately came over,

    ("「ちょっとちょっと！	あんたたち、なんでもう出てくんのよ！」",
    ""),
    # "And now, silly! Why are you guys already leaving?"

    ("「なんでって…、ふたりだけじ痰ゃ盛關り上がんないだ」",
    ""),
    # "Why...they're the only ones who have phlegms are so good that they're not going to get any more b."

    ("「盛關り上がんないと困骰るのよっ、こっちは」",
    ""),
    # "If you don't get too close, it's a problem, this one."

    ("「なにが困骰るんだよ」†",
    ""),
    # "What's that bothering you?"

    ("「っとにあんたたちは人がわざわざお膳立てしてやってるのに、どーしてそうなのよっ！」",
    ""),
    # "You guys are trying to set up and tamper with it, so what's that about?"

    ("なんだかよくわからないが、志保はえらい浮剣旧幕でまくし立てる。",
    ""),
    # I don't really understand why, but Shiho puts it up with the old curtain of the great Ukiken.

    ("「なんなの！？	あんたたち一生そうやってるつもり！？	いーかげん人の苦労燔も考えなさいよっ！」",
    ""),
    # "What is it?! You guys have been doing this forever!? Think about the hardships of people who are bastards!"

    ("「おい、さっきからなに怒ってんだよ」†",
    ""),
    # "Hey, what have you been mad for a while now?"

    ("「うっさいわね！	言えるならとっくに言ってるわよっ！",
    ""),
    # "That's annoying! If I could say it, I'd say it a long time ago!

    ("もー頭にｗたわ！	死ぬまでやってなさい！	バイバイッ！！」",
    ""),
    # It's a lmao in my head!Tamper until you die!Bye bye!!”

    ("一息にそこまで言うと、志保は背をｌけて一目散に駆け出して行った。†",
    ""),
    # After giving that much, Shiho leaned back and leaned forward.†

    ("「お、おい、志保」",
    ""),
    # "Oh, hey, Shiho."

    ("どうなってるんだ？",
    ""),
    # What's going on?

    ("あかりに聞こうと、オレは振關り返った。†",
    ""),
    # I called back to ask Akari.†

    ("「ち痰ゃん、追いかけてあげて…」",
    ""),
    # "Ah, chase me out..."

    ("なにも聞く前に、あかりはそう言った。",
    ""),
    # Akari said before asking what he was.

    ("「追いかけてどうすんだよ」",
    ""),
    # "What will you do after chasing?"

    ("「………」",
    "…"),
    # "..."

    ("今度はなにも言甯わない。†",
    ""),
    # This time, nothing can be said about the hot topic.†

    ("オレは｢｢。",
    ""),
    # I'm '.

    ("Ａ	追いかける。閨",
    ""),
    # A chase.Relax

    ("Ｂ	追いかけない。閨",
    ""),
    # B Don't chase.Relax

]