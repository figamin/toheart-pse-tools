# Generated array of Japanese text with English comments
# Source file: 08DE.txt

text_array = [
    ("（戟レミィ	好感度イベントＰＳ）閨（冝放課繪後に撃レミィが出ない冾日）閨（一梭時間目の休み桙時間）†",
    ""),
    # (Great Remie Likeability Event PS) (Day day when Ekiremie does not appear after school) (Great Remie is closed for the first hour) †

    ("「Ｈ奄ｉ！	」",
    ""),
    # "Hi Amai!"

    ("休み桙時間に撃レミィが竄やってきた。",
    ""),
    # At the break, Eki Remy came to tamper with him.

    ("「おう、戟レミィ」",
    ""),
    # "Oh, oh oh oh oh oh oh oh oh oh oh oh oh oh oh oh oh oh oh oh oh oh oh oh oh oh oh oh oh oh oh oh oh oh oh oh oh oh oh oh oh oh oh oh oh oh oh oh oh oh oh oh oh oh oh oh oh oh oh oh oh oh

    ("「、あのね、今晩家でパーティするの」",
    ""),
    # "Um, I'll have a party at home tonight."

    ("「へえ。なんの？」†",
    ""),
    # "Whoa. What?"

    ("「なんで烽もないｃ。",
    ""),
    # "Why don't you have a hot pot?

    ("普通のパーティ」",
    ""),
    # A normal party

    ("「俗にいう、晩サンラって竄やつか」",
    ""),
    # "Is it common to say that night sunra is tampering?"

    ("「うん。それでね、に烽も遊びにｗてほしいの」†",
    ""),
    # "Yes. And then, I hope you'll play with me lol too."

    ("「オ激レに？」",
    ""),
    # "Are you so crazy?"

    ("「うん」",
    ""),
    # "Yeah"

    ("コクンとうなずく撃レミィ。",
    ""),
    # Ekimi nods in a throbbing manner.

    ("ア＜メ潟リカンなホームパーティにオ激レを招待してくれるのか…。",
    ""),
    # A: Will they invite me to a home party that's so good?

    ("「ほかに誰か行くのか？」†",
    ""),
    # "Is there anyone else going?"

    ("「ううん。アタシの家族だけだｃ」",
    ""),
    # "Yeah, it's just my family."

    ("「ふ〜ん…。で烽もさ、そういうのって、近葛所の人とか呼んだりしねえのか？」",
    ""),
    # "Hmm... And you know, don't you call someone from Chikazusho?"

    ("「うん。…で烽も、誘って烽も断迺られち痰ゃうの。ニッポンの人、忙しいデス…」",
    ""),
    # "Yes... And I'm sure I'll be invited and even the hottest bait. Japanese people are busy..."

    ("「ああ、それはだな、用沫事があるとかじ痰ゃなくてな、冝日本人は薰基本的に近葛所のつき№合いがヘタってことだｂ」†",
    ""),
    # "Ah, that's right. There's no need to worry about droplets, and Japanese people are basically not good at the slugs of the chinese kudzusho."

    ("「ヘタなの？」",
    ""),
    # "Are you bad?"

    ("「冝日本人は義搴理がないと動かねえんだよ」",
    ""),
    # "The Japanese people can't move without any rationality."

    ("「％しいデス」",
    ""),
    # "Main Death"

    ("「オ激レに烽もな…」",
    ""),
    # "I'm not too hot..."

    ("「、義搴理ないの？」†",
    ""),
    # "Is there no reason to do it?"

    ("「オ激レか？	オ激レは｢｢」",
    ""),
    # "Are you so excited? I'm so excited

    ("Ａ	義搴理はたっぷりあるぜ。閨",
    ""),
    # A There's plenty of sensibility.Relax

    ("Ｂ	あるけど、予阯定があってな。閨",
    ""),
    # B There is, but there is a preliminary decision.Relax

]