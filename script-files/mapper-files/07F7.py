# Generated array of Japanese text with English comments
# Source file: 07F7.txt

text_array = [
    ("（琴音	回秤数イベント	ＰＳ）閨（選択綜	いっしょに帰うぜ）†",
    ""),
    # (Kotone, the balance event PS) (Select So, I'll return to the simulator)†

    ("「いっしょに帰うぜ」",
    ""),
    # "I'll return to the ephemeral."

    ("「…あ、はい」",
    ""),
    # "...Ah, yes."

    ("琴音ち痰ゃんはこくんとうなずいた。†",
    ""),
    # Kotone Chisho-chan nodded with Ko-kun.†

    ("・",
    ""),
    # ・

    ("・",
    ""),
    # ・

    ("・†",
    ""),
    # ・†

    ("「あっ…」",
    ""),
    # "Ah..."

    ("ふたりで歩いてると、琴音ち痰ゃんが小さな声をあげた。",
    ""),
    # As the two of them were walking around together, Kotone Chisho-yan let out a small cry.

    ("「え、なに？」†",
    ""),
    # "What?" †

    ("「…猫です」",
    ""),
    # "...it's a cat."

    ("「ああ」",
    ""),
    # "ah"

    ("琴音ち痰ゃんの見てる方に、なるほど、秩白地にモ茶色のぶちの入った猫が歩いている。",
    ""),
    # As Fukukata watches Kotone Chisho-yan, I see, a cat with brown thorns on a white background is walking around.

    ("そっと近寄驫ると、琴音ち痰ゃんはかがみ桙込んで右濶手をひらひらさせた。†",
    ""),
    # He gently approached him, and Kotone Chisho-chan bent over and flicked his right hand.†

    ("おっ、猫が反梍応した。",
    ""),
    # Oh, the cat has responded.

    ("ニャャ〜ン。",
    ""),
    # Meow.

    ("一声鳴いて、猫が琴音ち痰ゃんに近づく。",
    ""),
    # A cry of voice made the cat approach Kotone Chisho-chan.

    ("足元までやってきて、体をこすりつけた。†",
    ""),
    # He began to tamper with me to my feet and rubbed my body.†

    ("ニャャ〜ン。",
    ""),
    # Meow.

    ("「可愛い…。この子、人になれてますね」",
    ""),
    # "Adorable... This girl has become a human being."

    ("「轣らしいな」",
    ""),
    # "Likes"

    ("おっ、今度はこっちへ寄ってきたぞ。†",
    ""),
    # Oh, this time I've come over here.†

    ("オレの足に体をすりつけ始めた。",
    ""),
    # I began to rub my body against my hardcore legs.

    ("諱よく見骭ると、本末当に飼い猫逕らしくこぎれいな感じだ。",
    ""),
    # If you look closely at his name, he looks very neat and sleek, as if it were a pet cat.

    ("しかし、観察してると猫はさっさとオレから離齬れて琴音ち痰ゃんの方へ戻ってしまった。",
    ""),
    # However, as I was observing, the cat quickly separated from my slut and returned to Kotone's Symptomy's fortune.

    ("「ははっ、なんか凾嫌甯われてるなぁ、オレ」†",
    ""),
    # "Haha, I'm getting disliked, I'm super excited" †

    ("「そんなことないですよ。",
    ""),
    # "That's not the case.

    ("わたし、猫に好かれるおまじない、知ってますから。きっとそのせいです」",
    ""),
    # I know of a spell that cats like.It's probably because of that."

    ("猫をやさしく抱き上げると、琴音ち痰ゃんは猫にほおずりした。",
    ""),
    # As he gently hugged the cat and lifted it up, Kotone Chisho-chan squealed at the cat.

    ("「おまじない？	なにそれ、そんなもんあるのか？」",
    ""),
    # "A spell? What, is there such a hot spring?"

    ("「はい」†",
    ""),
    # "yes"†

    ("「どうやるんだ？」",
    ""),
    # "How are you going to tamper with it?"

    ("「簡単ですよ。目を見なければいいんです」",
    ""),
    # "It's easy. You don't have to look at your eyes."

    ("「目って、猫の？」",
    ""),
    # "Are your eyes cats?"

    ("「はい」",
    ""),
    # "yes"

    ("「目を見骭ると、凵嫌甯われるのか？」",
    ""),
    # "If you look into your eyes, will you be disgusted?"

    ("「はい」†",
    ""),
    # "yes"†

    ("「なんで？	目を見て話せ、とか言うだ？」",
    ""),
    # "Why? You say, look in your eyes, talk, b?"

    ("それは人間のことだけどな。",
    ""),
    # That's about humans though.

    ("「わたしたちはそうですが、猫同士は目が№合うと、それがケンカの№合図になるんです。人間とはケンカしませんが、やっぱり苦闍手なんですよ」†",
    ""),
    # "As we do, when cats meet their eyes, that's the signal to fight. We don't fight with humans, but we're still struggling."

    ("「ふ〜ん。つまり、琴音ち痰ゃんはそいつと目を№合寘わせないようにしてんのか」",
    ""),
    # "Hmm. In other words, Kotone Chisho-chan doesn't let him and his eyes go together?"

    ("「はい」",
    ""),
    # "yes"

    ("「…物知闥りだなぁ、琴音ち痰ゃんは」",
    ""),
    # "...You're knowledgeable, Kotone Chishonan."

    ("っていうより、ムツゴ鴻ロウさん並だ。†",
    ""),
    # Rather, he's about as much as Mutsugo Kourou-san.†

    ("ま、なんにせよ、琴音ち痰ゃんは超がつくほどの猫好きらしいな。",
    ""),
    # Well, in any case, Kotone Chisho-chan seems to be a huge fan of cats.

    ("Ａ	オレも猫が好き。閨",
    ""),
    # A. I also like cats.Relax

    ("Ｂ	オレは犬が好き。閨",
    ""),
    # B I love dogs.Relax

    ("Ｃ	オレは猫が凾嫌い。閨",
    ""),
    # C I hate cats.Relax

]