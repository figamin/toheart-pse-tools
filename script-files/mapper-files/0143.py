# Generated array of Japanese text with English comments
# Source file: 0143.txt

text_array = [
    ("（あかり	好感度イベント	２）閨（好感度が	１４以繹上	のとき）閨（２桙時間目）閨（教室を空っぽの状態にする、体迹育の授業のため）閨（時計を進める）閨（２桙時間目の休み桙時間）閨（渡闢り廊下）閨",
    ""),
    # (Akari Likeability Event 2) (When the Likeability is above 14) (2nd Floor Time) (For physical education classes to empty the classroom) (Preparing the clock) (Break time in the second Floor Time) (Counter corridor) (Counter Time)

    ("＊＊＊	時計件搶理漉注意	＊＊＊†",
    ""),
    # *Polishing* Polishing* Watch out for smearing polishing* Polishing* Polishing* Polishing*†

    ("２桙時間目の休み桙時間。",
    ""),
    # The second fence time is closed.

    ("朝っぱらからのきつい体迹育が終尞わり、教室へと戻ってくる途湯、",
    ""),
    # The harsh physical education that started early in the morning has ended, and the bathhouse returns to the classroom.

    ("「あっ、！」",
    ""),
    # "Ah,!"

    ("そんな声に呼ばれて振關りｌくと、渡闢り廊下から志保が駆け寄ってきた。†",
    ""),
    # As I was called by such a voice, Shiho came up to me from the corridor.†

    ("「やったーっ、いいとこにいるじ痰ゃない！」",
    ""),
    # "Yay, there's no phlegm in the good place B!"

    ("「なんだお前、そんな格好のまま。とっととｒ替えてこいよ」",
    ""),
    # "What is it, you're dressed like that. Let's just change it out quickly."

    ("「それどこじ痰ゃないのよ、蛛大変なのよ！」",
    ""),
    # "Are you phlegmy, it's tough!"

    ("「蛛大変？	またなにかしょーもない薰情報でも聞きつけてきたか？」",
    ""),
    # "Damn it hard? Have you heard of Hoo again with some kind of silly information?"

    ("オレがくだらなそうに笑って言うと、◯",
    ""),
    # When I laughed sloppyly,

    ("「ばかっ、そんなんじ痰ゃないわよっ！	本末当に大変なんだから！」",
    ""),
    # "Don't you, that kind of phlegm! It's really tough to be honest!"

    ("と、いつになくマジな顔で突っ返された。",
    ""),
    # He thrusts back with an unusually serious look on his face.

    ("「なんだよ、どうしたってんだ？」",
    ""),
    # "What, what's wrong?"

    ("「あかりが、あかりがっ｢｢」",
    ""),
    # "Akariga, Akariga" "

    ("「…あかりが、どうかしたのか？」†",
    ""),
    # "...What happened to Akari?"

    ("「蛛大ケガしたのよっ！」",
    ""),
    # "Dr you got seriously injured!"

    ("「蛛大ケガ气ぁ？」",
    ""),
    # "Damn you seriously injured?"

    ("志保は大きくうなずいた。",
    ""),
    # Shiho nodded loudly.

    ("「もしかしたら怩骨を折ったかも知れないの！	下濶手したらもう二度と歩けなくなるかも…」",
    ""),
    # "Maybe he's been exhausted or even a hot spring can't tell you! If you do it, you'll never be able to walk around again, so you'll probably be able to walk again..."

    ("「な、なに言ってんだよ、おまえ…」",
    ""),
    # "What, what are you talking about, you..."

    ("「ううっ、以前から鈍くさいとは思っていたけど、まさか、あんなことで…」†",
    ""),
    # "Uhh, I've always thought it was dull, but I can't believe it was like that..."

    ("「おいっ、あいつがいったいどうしたって？	もっと詳しく説明しよ」",
    ""),
    # "Hey, what on earth is he? Let me explain in more detail."

    ("オレは身を乗闖り出して言った。",
    ""),
    # I said, leaning back.

    ("あ、あかりが大ケガしたって！？",
    ""),
    # Oh, Akari got seriously injured!?

    ("どういうことだ！？",
    ""),
    # What does that mean??

    ("「うん、じつは今厓日の体迹育は砲丸滑投げだったんだけど…」",
    ""),
    # "Yeah, actually, the physical education on the day of the day was shot-skid throwing..."

    ("「梶投げた砲丸に魔当たったのか！？」†",
    ""),
    # "Did the shotgun thrown by the kaji hit the demon?!"

    ("「ううん、べつに」",
    ""),
    # "Uh, ugh, ugh."

    ("志保はさらりと言った。",
    ""),
    # Shiho said casually.

    ("「そんなことはなかったわ」",
    ""),
    # "That wasn't the case, right?"

    ("「なんだよ…」",
    ""),
    # "What is it..."

    ("「でも、今厓日はたまたま、あたしたちの班が後片づけの魔当番だったの。で、あかりたちと重たい砲丸を片づけてたんだけど…」",
    ""),
    # "And now, on the day, our team happened to be on duty to clean up afterwards. So we had been cleaning up the heavy shotgun with Akari and the others..."

    ("「氏落っことしたのか！？」†",
    ""),
    # "Did you fall for Mr.?!"

    ("「ううん、べつに」",
    ""),
    # "Uh, ugh, ugh."

    ("「………」",
    "…"),
    # "..."

    ("「ち痰ゃんと片づけたわよ」",
    ""),
    # "I've cleaned up a little smug"

    ("「だったらなんだよ、はやく言えよっ」",
    ""),
    # "If that's the case, say it."

    ("「なによぉ、詳しく説明しって言ったのはあんたでしょーが」",
    ""),
    # "What, you said, "You were the one who explained it in detail and said, "b."

    ("「詳しく、簡炎潔に言え！」",
    ""),
    # "Tell me more clearly and simply!"

    ("「だから、その後で｢｢」†",
    ""),
    # "So, after that, ""†

    ("そのとき。",
    ""),
    # At that time.

    ("「あっ、ち痰ゃん」",
    ""),
    # "Ah, sputum."

    ("声のほうをｌくと、クラスの翌女生徒に肩を借りたあかりが、ヒョコヒョコと片足を引きずりながら歩いていた。†",
    ""),
    # When Akari leaned her shoulders at the female student the following day after the party, she was walking around, dragging one leg.†

    ("「おい、あかり。お前、蛛大ケガしたって…」",
    ""),
    # "Hey, Akari. You're seriously injured..."

    ("「え、そ、そんな大袈裟なモンじ痰ゃないよ。ちょっとハードルにつまずいて足首ひねっただけだから…」",
    ""),
    # "What, that exaggerated b-mon, you can't sputum. I just tripped hard and twisted my ankle..."

    ("「けど、志保のрヤツは怩骨を折ったって…」",
    ""),
    # "But Shiho's guy is broken a bit of a pain..."

    ("「折った『かも』って言ったでしょ。折ったなんて断言はしてないわよ」",
    ""),
    # "I said, 'I've broken it too,' and I'm not saying that I broke it."

    ("「おめーな…」†",
    ""),
    # "Ahhh..." †

    ("「ち痰ゃん、心配してくれたの？」",
    ""),
    # "No way, did you worry?"

    ("「いや、そこにいるバカが、二度と歩けないとか抜かすからさ…」",
    ""),
    # "It's because the idiots there will never be able to walk again and they'll just let you go..."

    ("「誰がバカですって？」",
    ""),
    # "Who's the idiot?"

    ("「おめーだよ」",
    ""),
    # "Congratulations."

    ("オレははっきりと言い切った。†",
    ""),
    # I was completely clear about it.†

    ("「とこで、いまから保瀦健室行くのか？」",
    ""),
    # "At this point b, are you going to the Hole Inn Office now?"

    ("「うん、п私は平気だって言ったんだけど…」",
    ""),
    # "Yeah, I said I'm fine though..."

    ("「だめよ、あかり。軽いネンザでもち痰ゃんと冷笳やしてもらわなくち痰ゃ」",
    ""),
    # "No, light. You should just use a light noodle dish and squish it with a light noodle dish and squish it in a cool, cold pancake."

    ("肩を貸してる友だちが言った。",
    ""),
    # My friend who was lending her shoulder said.

    ("「そうだけど、みんなも≠早くｒ替えないと、氈次の授業に遅れち痰ゃうよ」",
    ""),
    # "That's right, but if everyone has to change the number of hot springs too soon, they'll end up being late in the next class."

    ("「そんなのどうでもいいって」†",
    ""),
    # "How about that, it's okay to have hot water."

    ("「なんなら、オレが連齪れてこうか？」",
    ""),
    # "In fact, should I keep coming back forth?"

    ("「え？」",
    ""),
    # "picture?"

    ("オレの提案に、あかりは驚いた顔をした。",
    ""),
    # Akari looked surprised at my proposal.

    ("「肩貸すぐらい、オレひとりいり痰ゃ十分だ」",
    ""),
    # "It's enough to lend my shoulders, and I have to go through all the phlegms."

    ("「でも…」†",
    ""),
    # "And the hot spring..." †

    ("「いいじ痰ゃない。そう思ってあたしもこいつを捕まえたんだから。こいつがなにかの役に立つことなんて、そうそうないんだし」",
    ""),
    # "It's fine, there's no phlegm. I also caught this guy because I thought so. It's rare that this guy is of any use."

    ("「おめーはだまって」",
    ""),
    # "Temee shut up b."

    ("「なによーっ！」†",
    ""),
    # "What!"

    ("「ま、そういうことだ。保瀦健室までけっこう翌距離あるしな。ほれ、つかまれ」",
    ""),
    # "Well, that's what it is. It's quite a distance from the Hole Inn room the next day. Come on, grab me."

    ("「うん、じ痰ゃあ…」",
    ""),
    # "Yeah, sputum..."

    ("うなずくと、あかりは片足で移動し、オレの肩につかまった。†",
    ""),
    # Nodding, Akari moved on one foot and grabbed her by my serpent shoulder.†

    ("「それじ痰ゃ、縺後はまかせたわよ」",
    ""),
    # "That's the phlegm, I'll leave it to you after it's shaking."

    ("「へいへい」",
    ""),
    # "Hey hey"

    ("「じ痰ゃねー」",
    ""),
    # "You're phlegmy."

    ("女性徒の背白を押して、志保は≠早足でもとｗた道を戻っていった。†",
    ""),
    # With the support of the bathing women in the bath, Shiho quickly walked back along the path he had already seen.†

    ("残されたのは、オレとあかりだけ。",
    ""),
    # All that was left was me and Akari.

    ("「………」",
    "…"),
    # "..."

    ("「………」",
    "…"),
    # "..."

    ("辺阨りを見回す。",
    ""),
    # I look around the baekket.

    ("「諱よし、誰熬もいねーな」",
    ""),
    # "Okay, no one can."

    ("「え？」†",
    ""),
    # "picture?"†

    ("「ほらよ…っと！」",
    ""),
    # "Come on..."

    ("「あっ…」",
    ""),
    # "Ah..."

    ("声とともに、オレは不意打ち気味に両腕であかりを抱き上げた。†",
    ""),
    # With both my voice and Hou, I held Kari with both arms and raised her arms as if to surprise me.†

    ("「、ち痰ゃん…！？」",
    ""),
    # "Are you sphemous...!?"

    ("「誰熬も見てねーし、このほうが≠早いだ？」",
    ""),
    # "No one has seen it, so this is faster than that?"

    ("「………」",
    "…"),
    # "..."

    ("「とは言っても、やっぱ誰かに見迪られるのはハズいからな、いちおう、ひとけのないとこを通って行くか」",
    ""),
    # "That said, even if you're not sure if someone else will pass you through a cousin."

    ("「…う、うん」†",
    ""),
    # "...Uh, yes" †

    ("オレは、他の生徒がいないか気にしながら、あかりを抱えて歩き出した。◯",
    ""),
    # I started walking around with Akari intently, worrying about whether there were other students there.◯

    ("「ハードルに蹴っつまずいてコケたって？」",
    ""),
    # "You mean you stumbled and stumbled on hard crying?"

    ("「…うん」",
    ""),
    # "…Yeah"

    ("「ったく、梶相変﨑わらず鈍くさいね、お前は」",
    ""),
    # "At that's it, Kaji Ai Henzaki is so dull, you."

    ("「そう…かな？」",
    ""),
    # "Is that so...?"

    ("「なんだ、自覚してねーのか？」",
    ""),
    # "What, you don't realize it?"

    ("「うっ、そ、そり痰ゃ、少しは…してるけど」",
    ""),
    # "Uh, I've got a little sputum, though."

    ("「はは」†",
    ""),
    # "mother"†

    ("「………」",
    "…"),
    # "..."

    ("「………」",
    "…"),
    # "..."

    ("「………」",
    "…"),
    # "..."

    ("「………」†",
    ""),
    # "..."†

    ("保瀦健室までの廊下を歩く。",
    ""),
    # I walked down the hallway to the Hole Inn.

    ("あかりはｎ囲を気にしてるのか、少し赤くなった顔をやや内側にｌけていた。†",
    ""),
    # Akari probably worried about the n-circumference, but her slightly red face was sniffing, tampering, and sniffing inside her.†

    ("もうすぐ保瀦健室にｒくというとき、ずっと無言だったあかりが、小声で言った。",
    ""),
    # Akari, who had been silent for so long, said in a whisper as he was about to go to the Horen Infirmary.

    ("「…ねえ、ち痰ゃん」",
    ""),
    # "...Hey, sputum."

    ("「あん？」†",
    ""),
    # "bean paste?"†

    ("「…п私、重いでしょ？」",
    ""),
    # "...I'm heavy, slut?"

    ("「いや」",
    ""),
    # "It's tampering."

    ("「…最近、少し太っち痰ゃったんだ」",
    ""),
    # "...I've had a little thick phlegm lately."

    ("「ふーん」",
    ""),
    # "Hmm."

    ("「腕、諱疲齡れない？」†",
    ""),
    # "Are you tired of your arm?"

    ("「ばーか」",
    ""),
    # "Oh no?"

    ("「………」",
    "…"),
    # "..."

    ("「ぜんぜん軽いよ」",
    ""),
    # "It's light at all."

    ("「…うそばっか」",
    ""),
    # "...I'm lying."

    ("抱えるオレの腕に、あかりはそっと自分の手を重ねた。◯",
    ""),
    # Akari gently placed her own hands on my hardcore arms.◯

]