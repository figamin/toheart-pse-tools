# Generated array of Japanese text with English comments
# Source file: 04E6.txt

text_array = [
    ("（志保	好感度イベントＰＳ）閨（選択綜	いいぜ）",
    ""),
    # (Shiho Likeability Event PS) (Selection So Iize)

    ("「いいぜ。どの辺までか知迺らねえけど」",
    ""),
    # "That's fine. I don't know where it goes."

    ("「諱よし。",
    ""),
    # "I'm fine with you.

    ("んじ痰ゃ行こか〜」",
    ""),
    # Let's go to the phlegm" 

    ("志保と他愛ないし痰ゃべりをしながら、駅前までやってきた。",
    ""),
    # I drove all the way to the station, trivial, phlegmy cattle with Shiho.

    ("「おっ、あれ見てよ」",
    ""),
    # "Oh, look at that."

    ("志保が指差す先を見骭ると、ギターを抱えた兄ち痰ゃんがなにやら聞き覚えのない曲をひいている。",
    ""),
    # Looking at the point Shiho was pointing, his brother Chisho-yan, holding a guitar, was playing a song he didn't know about.

    ("駅前とか人通闥りの多い場鰹所には、ああいう連中は珍しくない。",
    ""),
    # Those kinds of people are not uncommon in front of stations or in bonito spots where there are many people.

    ("「あれがどうかしたのか？」",
    ""),
    # "What happened to that?"

    ("「ちょっと聞いてくるから、あんたはここで待ってて」",
    ""),
    # "I'll ask you, so please wait here."

    ("そう言うと志保は、ギター弾きの兄ち痰ゃんの方へ歩き出した。",
    ""),
    # With that in mind, Shiho started walking towards Fukukata, the brother of the guitar player.

    ("「おい、聞くって、あの兄ち痰ゃんの曲をか？」",
    ""),
    # "Hey, listen to that big brother, is that song?"

    ("「瘁違うわよ。歌わせてくれるかってこと」",
    ""),
    # "It's a different roar. I'm going to let you sing."

    ("「は？」",
    ""),
    # "teeth?"

    ("歌わせてくれるかって？",
    ""),
    # Will you let me do it?

    ("あいつ、まさかここで歌うつもりか！？",
    ""),
    # Is that Tsukamori singing here??

    ("あぜんと見迪守骼るｌこうで、志保は兄ち痰ゃんと話し始めた。",
    ""),
    # As Shiho began talking to his brother, Symphomaniac.

    ("見撃、兄ち痰ゃんも困った顔をしてるじ痰ゃないか。",
    ""),
    # His brother, Sphinja, has a troubled look on his face, so isn't it?

    ("あ…、兄ち痰ゃんが自分の鞄から本を取り出した。",
    ""),
    # Oh... my brother, Chisho-chan, took a book from his own bag.

    ("ぺらぺらとページをくって、自分の足元に広げ",
    ""),
    # Slim down the page and spread it on your feet

    ("る…。",
    ""),
    # I'm...

    ("げっ、志保のрヤツ、閨手を振ってるぞ。",
    ""),
    # Wow, Shiho's guy is waving his hand.

    ("あの兄ち痰ゃん、ＯＫしたのかぁ？",
    ""),
    # That brother, Sphinja, did you get it?

    ("「１番。長岡志保、歌いま〜す！」",
    ""),
    # "No. 1. Nagaoka Shiho, I'm singing!"

    ("…あいつ、とうとう始めやがった…。",
    ""),
    # ...That guy finally started tampering with him...

    ("志保の歌堰@兄ち痰ゃんが弾き始めたその曲は、前奏ではよくわからなかったけど、志保の歌い出しでようやくわかった。",
    ""),
    # Shiho's song that Brother Symphonyan started playing was not well understood in the prelude, but Shiho's song started to come up with a sung story.

    ("…たしか『それぞれの未沫あしたへ』って曲だったな。",
    ""),
    # ...I'm sure it was a song called "Each of Misato Ashita."

    ("あいつ、ス鴻ロー系で攻めてきたか…。◯",
    ""),
    # He's been attacking with a S-Hong-Rou type...◯

    ("あーあ、歌っちまったよ…。",
    ""),
    # Ah, I sang it...

    ("＝普段気にもとめないのに、制梵服の翌女子ｎ高生が歌ってるとなると話は別だ。",
    ""),
    # =It's a different story when it comes to singing by a high school girl who usually doesn't look forward to its own personality, but the next female uniform is wearing a Sanskrit uniform.

    ("しかもマイクもエコーもなしで、これだけ声が通れば、人を集めるのも簡単だう。",
    ""),
    # However, there is no hot water, no microphone, or echo, and if the voices are so incomprehensible, it's easy to gather people.

    ("何人かが物珍しそうに足を止めて、今じ痰ゃ結構な人垣ができている。",
    ""),
    # Several people stopped in a strange way, and now there is a considerable crowd of people who have just come to understand the phlegm.

    ("珍しいからだけじ痰ゃない。",
    ""),
    # It's rare, so there's no phlegm.

    ("それなりに聞く価値があるほどの歌声だからだ。",
    ""),
    # That's because it's a voice that's worth listening to.

    ("悔しいが、志保のрヤツは抜群に歌がうまい。",
    ""),
    # It's frustrating, but Shiho's guy is extremely good at singing.

    ("…今すぐ芸能界にデビャーしたとしても、歌唱派アイドルで通用するかもしれない。",
    ""),
    # ...If he's just advertised into the entertainment industry right now, it's probably true that he'll be able to get the singular cry of singing.

    ("「おにーさん、つぎつぎ！」",
    ""),
    # "Onii, one after another!"

    ("気をよくした志保は、兄ち痰ゃんを急かした。",
    ""),
    # Shiho, who was so happy, rushed his brother, Sphinja.

    ("あのバカ…！",
    ""),
    # That idiot...!

    ("「おい志保」",
    ""),
    # "Hey Shiho"

    ("慌ててオレは志保に駆け寄驫る。",
    ""),
    # I hurriedly leapt over Shiho.

    ("「なに？	あんたも歌いたいの？」",
    ""),
    # "What? Do you want to sing too?"

    ("「わけねーだ。お前ひとりならともかく、オレという連齪れがいること忘齧れんなよ」",
    ""),
    # "There's no way you can. If you're alone, you'll be alone, so don't forget that you're going to have a series of intense tragedy."

    ("なにしその間、待たされてるだけなんだからな。",
    ""),
    # What's that b I'm just making you wait for that while.

    ("「だったらあんたも歌えばいいじ痰ゃん」",
    ""),
    # "If that's the case, you should just sing it, smug-yaa."

    ("「諱よくねー諱よ」",
    ""),
    # "Not well, your name is"

    ("「わかんないわね〜っ」",
    ""),
    # "I don't know, you're not sure."

    ("「わかんねえのはおめーだ。オレはお前みたく、人様に歌って聞かせるほどうまくねえんだよ」",
    ""),
    # "I don't know. I'm not good enough to sing to others like you, and I'm not good enough to sing to others."

    ("「あら、そういえばそうね。いま気づいたわ」",
    ""),
    # "Oh, come to think of it, right. I just realized it."

    ("「わかった？	だったらここで歌い続けるか帰驪るかｌめ。歌うんだったらオレは帰驪る」",
    ""),
    # "Do you get it b? If so, I'll continue singing here or go back. If I'm going to sing, I'll go back."

    ("「な、なによう。あんたに帰迢られち痰ゃしらけち痰ゃうじ痰ゃない」",
    ""),
    # "What? You're going to return to me, phlegm, phlegm, phlegm, phlegm."

    ("「じ痰ゃ、′まりだな」",
    ""),
    # "You're so phlegm, it's 'mai'."

    ("志保は『しょーがないわね』と兄ち痰ゃんにｌき直って、",
    ""),
    # Shiho said, "You're not in the guise," and replied with his brother, "You're not in the guise."

    ("「ごめんねぇ、おにーさん。連齪れが急ぐって言うから。あと、ありがとね〜」",
    ""),
    # "I'm sorry, you guys. I'm going to hurry up. Also, thank you."

    ("と手を振った。",
    ""),
    # He waved.

    ("兄ち痰ゃんの方熾も『俺の知ってる曲ならいつでも弾いてやるぜ』と気さくに手を振關り返してくれた。",
    ""),
    # Fukukata Taru, who is the brother of the song, also waved back to his heartfelt reaction, saying, "If it's a song I know, I'll play and tamper with him."

    ("「１曲だけだったけど、すっきりしたわぁ」",
    ""),
    # "It was only one song, but it's refreshing."

    ("「そらよかったな」",
    ""),
    # "That's good."

    ("「なんか病み付きになりそ翌＠」",
    ""),
    # "I'm addicted to you, and the next day @"

    ("「お前って、出初ったときから目立ってて、スター性？	っつーのがあったよな」",
    ""),
    # "You've been a star-like figure since you first appeared on the show, right?"

    ("「ま、そーね」",
    ""),
    # "Well, that's it."

    ("「オレが保証してやってもいいぜ？	おめえは将女、有名人になるってな」",
    ""),
    # "I'm hoping to get my guilty and tamper with it, isn't it okay? You're the owner, you're going to become a celebrity."

    ("「へえ、言うもんねぇ。あたしにрヤックでもおごれって言うの？」",
    ""),
    # "Wha, you say it's a hot spring. Are you saying that you should treat me to the hot springs as well?"

    ("「なんだ、おごってくれるのか？」",
    ""),
    # "What, will you pay me?"

    ("「ごじょーだんを。あたしの未沫はあんたに保証されなくても約束されてるのよ」",
    ""),
    # "An slut. My unspoiled droplets are not guaranteed to you, and the hot water is also promised."

    ("「けっ、諱よくもまぁそこまで言えるもんだな」",
    ""),
    # "Ah, you can really say that hot springs."

    ("「『志保ち痰ゃん自信』って呼んでもいいわよ」",
    ""),
    # "You call me 'Shiho-chan confidence' and it's good to hear the hot weather."

    ("「なんだそれ？	グラグラ艫ゆれるのか？」",
    ""),
    # "What is that? Will the banquet be shaking?"

    ("「おバカ、噤字が痰違うじ痰ゃないのよ」",
    ""),
    # "Idiot, the phlegm is different from the phlegm."

    ("「轣冗談だよ。お前の根注拠のない自信なんて、どっちみちグラグラ艫ゆれるんだうが」",
    ""),
    # "I'm kidding. Your unfounded confidence will just sway either way."

    ("「なんですってぇ？	あたしより面猪白いこと言甯わないでよっ」",
    ""),
    # "What is that? Don't say something more white than me."

    ("「おめーがつまんねえこと言ってるからだよ」",
    ""),
    # "It's because you're saying something boring."

    ("そんなふうにつべこべ言い№合って、オレと志保は札前で別れた。◯",
    ""),
    # I was so lazy that I agreed, and I and Shiho parted ways with him in front of the I-card.◯

]