# Generated array of Japanese text with English comments
# Source file: 04F2.txt

text_array = [
    ("（志保	ａ高感度型回秤数イベント）閨（選択綜	潤ワ潟リィけどパス）†",
    ""),
    # (Shiho a high sensitivity type scale event) (Shojo Junwagata Li but pass)†

    ("「潤ワ潟リィけど、パス」†",
    ""),
    # "Junwagata Li, but I'll pass"

    ("諱先約があるからな。",
    ""),
    # Because there is a name for the first time.

    ("迷嵂わず撃レミィとのデートを優譌先だ。†",
    ""),
    # Without a slut, he went on a date with Geki Remy.†

    ("「え〜っ！？	どうしてぇ？」",
    ""),
    # "Huh!? Why?"

    ("「だいたい４桙時間煌もしんどいだｂ？	オールナイトじ痰ゃなくて、＝普通のрヤツを見に行けばいいじ痰ゃねえか」",
    ""),
    # "Around 4-Fish Time Killing is also tough b? It's all night long, so it's okay to go and see the regular guy, right?"

    ("「だってオールナイト黹専用のチケットなのよ？	あんたと行きたいのよ？」†",
    ""),
    # "Because it's a ticket exclusively for All Night Monkeys? I want to go with you, right?"

    ("「終尞わって烽も電車の始発まで桙時間あるだｂ？	それがまたしんどい」†",
    ""),
    # "The end of the day, the train has time to go before the start of the train b? That's another tough one."

    ("「じ痰ゃあ、タクシーで｢｢」",
    ""),
    # "Ah, I'll take a taxi to say "

    ("「マジかよ？	下濶手すり痰ゃ１怩万近くかかるぞ」",
    ""),
    # "Are you serious? If you're going to get spurts, it'll cost nearly 10,000 yen."

    ("「う〜っ；」",
    ""),
    # "Uhh;"

    ("「もちｂんお前のおごりだｂうな？」",
    ""),
    # "Mochib, it's your treat, b, isn't it?"

    ("「う〜っ；」",
    ""),
    # "Uhh;"

    ("「何皷語だそり痰ゃ」",
    ""),
    # "What are you talking about, phlegm?"

    ("「う〜〜〜っ；」",
    ""),
    # "Hmm;"

    ("「うなると烽金が出てくるのか？」†",
    ""),
    # "If you growl, will you get some hot money?"

    ("「…志保ち痰ゃんの奄負けぇ」",
    ""),
    # "...Shiho, sphin-yan, loses Amami."

    ("「はははっ、残念だったな。また誘ってくれ」",
    ""),
    # "Hahaha, that's a shame. Please invite me again."

    ("「あ〜つまんない」",
    ""),
    # "Ah, it's boring."

    ("そう言って、志保は帰っていった。◯",
    ""),
    # With that in mind, Shiho left.◯

]